...
- PCM 等待期间预启动降噪子进程并一次性灌入约 0.8 秒静音:模型加载
   (Pi4 上 5~7 seconds)与 PCM 等待并行进行，转发真正启动时降噪进程已就绪，
...
- **电平表心跳 + 字节计数**:电平表每 HP_METER_HEARTBEAT_SECS 秒 (默认 15 seconds)报一次
...
- 开麦克风后放音被压掉 ("一开录音就不能放音")的应对:下行有**两条**通路，
  ...
  在两条死通路之间来回跳——A2DP ↔ Hands-Free 下行(SCO) 互顶 (最多 6)；
...
- 开关:HEADPHONE_ENABLE=0 只当麦克风；A2DP_SINK_ENABLE=0 不注册 A2DP (电脑只能
  ...
  心跳间隔 (0 = 只在档位变化时报)
- **提示音 (NOTICE_TONE=0 可关)**:三个时机各播一次，音型不同以便只凭耳朵区分——
  蓝牙连接 = 两声上行 (低→高)、降噪可用 = 三声上行短促、放音可用 = 两声下行 (高→低)。
  播放在耳机口:下行管道没跑时直接用一个短 aplay；管道在跑时把音型写进
  `/tmp/bt_notice_tone`，由下行电平表阶段混进正在播放的音频 (声卡被管道独占，
  再起 aplay 只会 EBUSY)。音型表见 NOTICE_TONES
"""
...
def save_config_key(key, value):
    ...
    if kept and kept[-1].strip():
        kept.append("")
    kept.append("# Written by %s --find-output (%s)"
...
# 提示音:蓝牙连接 / 降噪可用 / 放音可用三个时机各播一次，音型不同以便只凭
# 耳朵就能区分是哪一步就绪了。NOTICE_TONE=0 可整体关掉。
#   bt       = 两声上行 (低→高): 链路建立
#   denoise  = 三声上行短促:     降噪开始工作
#   playback = 两声下行 (高→低): 耳机放音通路可用
NOTICE_TONE_ENABLE = os.environ.get("NOTICE_TONE", "1").strip().lower() not in (
    "0",
    "false",
    "off",
    "no",
)
NOTICE_TONE_FILE = "/tmp/bt_notice_tone"   # 提示音请求文件 (见 play_notice)
NOTICE_TONE_RATE = 44100
NOTICE_TONE_GAP_MS = 60                    # 音型内部各音之间的静音间隔
NOTICE_TONES = {
    "bt": [(660.0, 110), (990.0, 190)],
    "denoise": [(880.0, 80), (1174.0, 80), (1568.0, 170)],
    "playback": [(1318.0, 110), (988.0, 110), (659.0, 190)],
}
...
_HAT_MODE = [False]
# 提示音相关的共享状态 (见 play_notice):
#   _PLAYBACK_DEVICE = 已选定的耳机口设备 (播提示音时直接用，不再重复探测)
#   _NOTICE_LOCK     = 直接播提示音时的互斥锁: 下行播放器启动前会先拿这把锁，
#                      避免"提示音占着声卡、播放器启动 EBUSY"的死结
#   _DOWNLINK_UP     = 下行播放管道是否正在跑 (跑着就改写提示音请求文件，
#                      由管道里的电平表阶段把音型混进正在播放的音频)
_PLAYBACK_DEVICE = [None]
_NOTICE_LOCK = threading.Lock()
_DOWNLINK_UP = threading.Event()
...
BUILD_ID = ("2026-09-13-01 English log messages + notification tones (Bluetooth "
            "connected / noise reduction ready / playback ready) on top of: "
            "playback device selection prefers a card other than the microphone's "
            "+ /etc/default/bt-mic config file + --find-output per-card test")
_SCRIPT_MTIME = [None]
...
def check_script_updated():
    ...
    if mtime > _SCRIPT_MTIME[0] + 1 and not _UPDATE_WARNED[0]:
        ...
        _SCRIPT_MTIME[0] = mtime
        print("  !! The script on disk was updated, but the current process is still running the old code:")
        print("     Please run sudo systemctl restart bt-mic (restart this program if it runs in the foreground)")
...
def run(cmd, check=False, timeout=60, verbose=True):
    ...
    try:
        ...
        if check and result.returncode != 0:
            print(result.stdout)
            print(result.stderr)
            raise RuntimeError(f"Command failed (returncode={result.returncode}): {cmd}")
        return result
...
def ensure_root():
    if os.geteuid() != 0:
        print("Not running as root, trying to restart automatically with sudo...")
...
def remove_paired_devices():
    ...
    if not result or result.returncode != 0:
        print("  !! Cannot read the pairing list (adapter not ready?); the link will drop after a system reboot")
        return
    for line in result.stdout.splitlines():
        parts = line.split()
        if len(parts) >= 2 and parts[0] == "Device":
            ...
            print(f"  -> Removed paired device {mac} (Windows auto-reconnect will fail)")
...
def forget_paired_devices():
    ...
    """
    print("=== Removing the pairing on the Raspberry Pi side to force the PC to re-pair and rediscover services ===")
    print("  The PC side needs no device removal: it will re-pair (PIN-less) and re-enumerate services while pairing")
    remove_paired_devices()
    set_discoverable(True)
    print("  -> Now click that device in the PC's Bluetooth settings to reconnect")
    print("  -> After re-pairing, 'Sound settings -> Output' will show 'Headphones (device name Stereo)',")
    print("     that is the A2DP music route (which sends PC audio to the 3.5mm headphone jack)")
...
def restore_default():
    ...
    if _RESTORED:
        return
    _RESTORED = True
    print_status("Restoring system defaults and disconnecting Bluetooth")
...
def setup_packages():
    ...
    if not missing:
        print_status("Dependency check passed (bluez/bluealsa already installed, skipping apt)")
        if shutil.which("bluealsa-cli") is None:
            print("  !! bluealsa-cli is missing: diagnostics and volume control are unavailable")
            print("     Please run sudo apt install bluez-alsa-utils (the microphone is unaffected)")
        return
    print_status(f"Checking and installing dependencies (missing: {', '.join(missing)})")
...
def disable_audio_servers():
    print_status("Disabling PulseAudio / PipeWire / oFono / ModemManager / hsphfpd")
    ...
    if leftover and (leftover.stdout or "").strip():
        print("  !! Conflicting processes are still running, please check manually:")
        print((leftover.stdout or "").strip())
    else:
        print("  -> No leftover audio/telephony service processes found")
...
def find_bluetoothd():
    ...
    if found:
        return found
    raise FileNotFoundError("bluetoothd not found")
...
def ensure_bluealsa_dbus(hci):
    if has_bluealsa_dbus_name():
        return True
    print("  !! org.bluealsa not detected, trying to restart the bluealsa service")
    run("systemctl restart bluealsa", check=False, verbose=False)
    time.sleep(2)
    if has_bluealsa_dbus_name():
        print("  -> org.bluealsa recovered")
        return True
    ba_path = shutil.which("bluealsa")
    if not ba_path:
        return False
    print("  !! Still unavailable, continuing with a direct start of the bluealsa daemon with explicit arguments")
...
def diagnose_uuid_conflicts():
    print("  -- Processes that may occupy the HFP/HSP UUID --")
    result = run(
    ...
    if result and (result.stdout or "").strip():
        if "--noplugin=audio,headset" in (result.stdout or ""):
            print("  -> bluetoothd has the built-in audio/headset plugins disabled")
        else:
            print(
                "  !! bluetoothd is not running with --noplugin=audio,headset, please check whether override.conf took effect"
            )
    ...
    if names and (names.stdout or "").strip():
        print("  -- Related service names on D-Bus --")
        print((names.stdout or "").strip())


def resolve_uuid_conflict(hci):
    print_status("Resolving the UUID conflict: stop the conflicting services and restart bluetooth/bluealsa")
    run("systemctl stop bluealsa 2>/dev/null || true", check=False, verbose=False)
    ...
    if not (check and "--noplugin=audio,headset" in (check.stdout or "")):
        print("  !! Note: bluetoothd is not running with --noplugin=audio,headset, it may be unable to release the UUID")
    ...
    if (
        ...
        print("  -> UUID conflict cleared, bluealsa can register HFP/HSP")
        return True

    print("  !! The UUID conflict still exists; the processes/services that may occupy the UUID are:")
    diagnose_uuid_conflicts()
    if journal and (journal.stdout or "").strip():
        print("  -- Recent bluealsa log --")
...
def configure_main_conf():
    ...
    print_status("Configuring /etc/bluetooth/main.conf (device class/discoverability/PIN-less re-pairing)")
    if not os.path.exists(BLUETOOTH_CONF):
        print("  !! main.conf not found, skipping (runtime bluetoothctl settings still take effect)")
        return
...
def configure_bluetoothd():
    print_status("Configuring BlueZ, disabling the built-in audio/headset plugins")
    ...
    if check and "--noplugin=audio,headset" in (check.stdout or ""):
        print("  -> bluetoothd has the audio/headset plugins disabled")
    else:
        print("  !! Warning: --noplugin=audio,headset not detected, a plugin conflict may still exist")
...
def log_bt_disconnect_reason():
    ...
    if hits:
        print("  -- BlueZ log (disconnect-related, used to tell who disconnected) --")
        for line in hits[-6:]:
            print(f"    {line}")
...
def configure_bluealsa():
    print_status("Configuring BlueALSA")
    ba_path = shutil.which("bluealsa")
    if not ba_path:
        raise FileNotFoundError("bluealsa not found, please make sure bluez-alsa is installed")
    ...
    if BLUEALSA_EXTRA_ARGS:
        ...
        print("  -> Appending bluealsa arguments (BLUEALSA_EXTRA_ARGS): %s" % BLUEALSA_EXTRA_ARGS)
    a2dp_candidates = bluealsa_a2dp_profile_name(ba_path) if A2DP_SINK_ENABLE else []
    if not A2DP_SINK_ENABLE:
        print("  -> A2DP_SINK_ENABLE=0: only HFP/HSP is registered (the PC can only play audio through the Hands-Free route)")
    ...
    for name in a2dp_candidates:
        ...
        if not restart_bluealsa():
            print(f"  !! Starting bluealsa with A2DP profile '{name}' failed, trying other names")
        ...
        if a2dp_sdp_ok is False:
            print(f"  !! profile '{name}' started successfully, but there is no Audio Sink record in the SDP,")
            print("     retrying with a different profile name")
            continue
        a2dp_profile = name
        break
    if a2dp_profile is None:
        ...
        restart_bluealsa()
        if a2dp_candidates:
            print("  !! Failed to register A2DP Sink, fell back to HFP/HSP only (microphone first)")
    _A2DP_REGISTERED[0] = a2dp_profile
    print(f"  -> Using arguments: {new_exec}")
    if a2dp_profile:
        print(f"  -> A2DP Sink registered ({a2dp_profile}): the PC can use the Raspberry Pi as headphones/speaker")

    result = run("systemctl is-active bluealsa", check=False, verbose=False)
    if result and result.stdout.strip() == "active":
        print("  -> BlueALSA service state: active")
    else:
        print("  !! BlueALSA service is not active, trying to start the Bluetooth HF/HS mode directly")
        run(
    ...
    if ensure_bluealsa_dbus(hci):
        print("  -> BlueALSA D-Bus ready (org.bluealsa)")
    else:
        print(
            "  !! BlueALSA D-Bus is still not ready, please check /tmp/bluealsa.log and systemctl status bluealsa"
        )
    ...
    if "UUID already registered" in journal_text:
        ...
        if "Couldn't register hands-free profile" in journal_text:
            print("  !! HFP/HSP UUID conflict detected, trying to clean up...")
            ...
            print("  !! A2DP UUID conflict detected (the headphone feature needs it), trying to clean up...")
            resolve_uuid_conflict(hci)
        else:
            # 只有 A2DP UUID 冲突，不影响 HFP/HSP 麦克风功能
            print("  -> A2DP UUID conflict only, it does not affect the HFP/HSP microphone route, you can continue")
    else:
        print("  -> No HFP/HSP registration conflict detected")

    # 关键验证:Windows 配对时只缓存当时能查到的服务。若 HFP 记录缺失，
    # Windows 永远不发起服务级连接 ("已连接但无法选为麦克风"的根因)
    sdp_ok = verify_hfp_sdp_record()
    if sdp_ok is False:
        print("  !! The local SDP is missing the Handsfree record, restarting bluealsa to re-register...")
        run("systemctl restart bluealsa", check=False, verbose=False)
        time.sleep(3)
        sdp_ok = verify_hfp_sdp_record()
    if sdp_ok is True:
        print("  -> Confirmed the local SDP contains the Handsfree service record (Windows can discover HFP)")
    elif sdp_ok is False:
        print("  !! The Handsfree SDP record is still missing after the restart: Windows will not see HFP when pairing,")
        print("     and the Raspberry Pi cannot be selected as a microphone. Please check journalctl -u bluealsa.")

    # A2DP Sink 记录同理:缺失时 Windows 只把它当免手持设备，声音设置里
    # 没有立体声输出项 (耳机口只能靠 HFP 下行出声，16kHz 单声道)
    if a2dp_profile:
        if a2dp_sdp_ok is True:
            print("  -> Confirmed the local SDP contains the Audio Sink record (the PC can use the Raspberry Pi as headphones/speaker)")
        else:
            # 上面校验收到了 None (缺 sdptool 或查询失败, 这里再补一次
            a2dp_ok = verify_a2dp_sdp_record()
            if a2dp_ok is False:
                print("  !! The local SDP is missing the Audio Sink (A2DP) record, restarting bluealsa to re-register...")
                run("systemctl restart bluealsa", check=False, verbose=False)
                time.sleep(3)
                a2dp_ok = verify_a2dp_sdp_record()
            if a2dp_ok is True:
                ...
                print("  !! The Audio Sink SDP record is still missing: Windows sound settings will not show a stereo output entry,")
                print("     the headphone jack can only use the Hands-Free downlink. Please check journalctl -u bluealsa.")
...
def start_pairing_agent():
    ...
    if _AGENT_PROC is not None:
        ...
        print("  -> Pairing agent running persistently (NoInputNoOutput), PIN-less pairing with Windows is ready")
    else:
        print("  !! Failed to start the pairing agent, it will be retried automatically later")
...
def ensure_pairing_agent():
    ...
    if _AGENT_PROC is not None and _AGENT_PROC.poll() is None:
        return
    print("  -> Pairing agent exited, restarting...")
    _AGENT_PROC = _spawn_pairing_agent()
    if _AGENT_PROC is not None:
        print("  -> Pairing agent restarted")
...
def recover_bt_adapter():
    ...
    print("  !! Bluetooth controller missing, trying to recover (restart hciuart / bring up hci0 / power on)...")
    run(
...
def wait_for_bt_adapter(timeout=90):
    print_status("Waiting for the Bluetooth adapter to be ready (this can take a few seconds at boot)")
    start = time.time()
    while time.time() - start < timeout:
        ...
        if bt_controller_present():
            print("  -> Bluetooth adapter is ready (Powered: yes)")
            return True
        ...
        recover_bt_adapter()
        time.sleep(3)
    print(f"  !! Bluetooth adapter not ready after {timeout} seconds, continuing to run (the background keeps retrying)")
    return False
...
def set_discoverable(on):
    if on:
        ...
        for attempt in range(3):
            ...
            if state == (True, True):
                print("  -> Bluetooth is now discoverable/pairable, waiting for Windows to connect")
                return True
            time.sleep(1)
            run("bluetoothctl discoverable on", check=False, verbose=False)
            run("bluetoothctl pairable on", check=False, verbose=False)
        state = discoverable_state()
        if state is None:
            print("  !! Bluetooth controller does not exist (adapter not ready?), it will be retried automatically later")
            recover_bt_adapter()
        else:
            print(
                f"  !! Discoverability settings did not take effect (Discoverable={state[0]}, Pairable={state[1]}, "
                "it will be retried automatically later"
            )
        return False
    else:
        run("bluetoothctl discoverable off", check=False, verbose=False)
        # pairable 保持开启:已配对设备仍然可以重连/重配对
        print("  -> Device connected, Bluetooth discoverable turned off")
...
def prepare_bt_state():
    print_status("Setting the Bluetooth device class and power")
    run("rfkill unblock bluetooth 2>/dev/null || true", check=False, verbose=False)
    wait_for_bt_adapter()
    run("bluetoothctl power on", check=False, verbose=False)
    hci = get_hci_name()
...
def wait_for_connection(timeout=CONNECTION_TIMEOUT):
    if timeout > 0:
        print_status(f"Waiting for Windows to connect (up to {timeout} seconds)")
    else:
        print_status("Waiting for Windows to connect (wait indefinitely, Ctrl+C to stop)")
    print("  PIN-less pairing: add a device in Windows Bluetooth settings and select the Raspberry Pi, no pairing code required")
    print("  If Windows still shows a PIN prompt, first 'Remove device' in Windows Bluetooth settings and add it again")
    print("  After connecting, go to Windows 'Sound settings -> Input' and select the Hands-Free/Headset device,")
    print("  only then will BlueALSA establish the HFP/SCO microphone route.")
    print("  To play sound through the reSpeaker 3.5mm headphone jack: go to Windows 'Sound settings -> Output'")
    print("  and select this device ('... Stereo' uses A2DP stereo, '... Hands-Free' uses the 16kHz route),")
    print("  both produce sound; if there is no stereo entry in sound settings, the device must be removed and re-paired.")
    start = time.time()
    last_notice = 0
    last_ensure = 0
    while True:
        ...
        if devices:
            print(f"  -> Device connection detected: {devices[0]}")
            return devices[0]
        if time.time() - last_notice >= 30:
            print("  Still waiting for Windows to connect... (the program keeps running)")
...
def dump_pcm_diagnostics():
    ...
    print("  -- PCM discovery diagnostics (raw BlueALSA output) --")
    for cmd in ("bluealsa-aplay -L", "bluealsa-cli list-pcms",
        ...
        if not shutil.which(cmd.split()[0]):
            ...
            continue
        res = run(cmd, check=False, timeout=8, verbose=False)
        print(f"  $ {cmd}  ->  rc={res.returncode if res is not None else '(timed out)'}")
        if res:
...
def request_hfp_profile(device, uuid="0000111e-0000-1000-8000-00805f9b34fb"):
    ...
    if res is None:
        print("    ConnectProfile: call timed out/failed (no output)")
        return False
    out = ((res.stdout or "") + " " + (res.stderr or "")).strip()
    tag = "all profiles" if uuid == "" else f"UUID {uuid}"
    print(f"    ConnectProfile({tag})  ->  rc={res.returncode} {out[:160]}")
    return res.returncode == 0


def find_working_pcm(device, timeout=90):
    print_status(f"Waiting for the BlueALSA SCO PCM to appear (this round, up to {timeout} seconds)")
    ...
    while time.time() - start < timeout:
        ...
        if round_no > 1 and not is_device_connected(device):
            print("  -> Bluetooth disconnect detected while waiting for the PCM")
        ...
        for pcm in candidates:
            ok, err = test_pcm(pcm)
            if ok:
                print(f"  ** Usable PCM found: {pcm}")
                return pcm
            if detailed:
                print(f"  Testing: {pcm}  ->  not usable{(':' + err) if err else ''}")
        ...
        if detailed:
            print(
                f"  (round {round_no}) HFP service-level connection (SLC): {'established' if slc_up else 'not established'}"
            )
            if slc_text:
                print(f"    {slc_text}")
            if slc_up:
                print("    SLC is up but the PCM still cannot be opened: the channel may be held exclusively by a leftover aplay,")
                print("    or Windows has not enabled recording yet (the SCO is only created after you select it in Sound settings -> Input).")
            else:
                print("    Windows has not initiated the HFP service-level connection (SLC) yet. It only initiates it for the")
                print("    services it cached at pairing time: if HFP is not in that cache, it will never initiate it (see the guidance below).")

        if not listed:
            print(
                f"  (round {round_no}) the list tool reported no PCM containing this machine's MAC, so the"
                " constructed names were probed directly."
            )
            ...
            print(
                f"  (round {round_no}) {len(candidates)} PCM candidates are all unusable: "
                "SCO is not up yet, or the channel is occupied by a leftover aplay"
            )

        # 每 5 轮 (约 15 seconds)轮换 UUID 重试 ConnectProfile，并顺手清掉被
        ...
        if round_no % 5 == 0:
            ...
            if kill_bt_profile_conflicts():
                print("    -> cleaned up the reappearing conflicting processes, restarting bluealsa to re-register the profile")
                run("systemctl restart bluealsa", check=False, verbose=False)
                time.sleep(3)
        ...
        if not slc_up and not forced_disconnect and time.time() - start >= 25:
            ...
            if HEADPHONE_ENABLE and a2dp_stream_active(device):
                print("  -> no HFP service-level connection (SLC) after 25 s, but A2DP is playing audio")
                print("     (the PC treats it only as headphones and has not enabled the microphone): not disconnecting deliberately, keep waiting")
                print("     if you need to use the microphone, select Hands-Free in Windows Sound settings -> Input")
            else:
                print("  -> no HFP service-level connection (SLC) after 25 s: deliberately disconnecting Bluetooth to trigger Windows")
                print("     to reconnect automatically and retry the Hands-Free service (Windows reconnects within seconds)")
                run(f"bluetoothctl disconnect {device}", check=False,
                    timeout=15, verbose=False)
                print("  -> if it is still the same after reconnecting, it means Windows' cached device services have no HFP:")
                print("     delete the device in Windows Bluetooth settings, and once this program shows it as discoverable")
                print("     add and pair it again (the Pi must already have HFP registered at the moment of pairing).")

        time.sleep(3)

    # 额外诊断
    print("  !! no usable PCM found in this round, requesting the HFP profile on the link again")
    request_hfp_profile(device)
    time.sleep(5)
    ...
    print("  !! still no PCM found. Please try:")
    print("     1. delete the device in Windows Bluetooth settings and pair it again;")
    print("     2. after pairing, open 'Sound settings' and select Hands-Free/Headset as the input device;")
    print("     3. open any recording app so that Windows really enables the microphone channel;")
    print("     4. check whether Windows sees the Pi as 'Headphones' instead of 'Speakers'.")
    if not diag_done:
        dump_pcm_diagnostics()
    return None
...
def write_headphone_status(**extra):
    ...
    if _A2DP_REGISTERED[0]:
        ...
        lines.append("a2dp_sink=(not registered)")
...
def test_alsa_playback(dev, rate=48000, channels=2):
    ...
    if not shutil.which("aplay"):
        return False, "aplay missing (please install alsa-utils)"
    cmd = (
...
def select_playback_device(mic_device=None):
    ...
    """
    if not shutil.which("aplay"):
        return None, "aplay missing (please install alsa-utils)"
    if PLAYBACK_DEVICE:
        ok, err = test_alsa_playback(PLAYBACK_DEVICE)
        if ok:
            return PLAYBACK_DEVICE, f"configured PLAYBACK_DEVICE={PLAYBACK_DEVICE}"
        print(f"  !! PLAYBACK_DEVICE={PLAYBACK_DEVICE} cannot be opened: {err}")
        print("     change the config: sudo nano %s or sudo python3 %s --find-output"
    ...
    for card, dev, label, same in ordered:
        ...
        print("  -> playback device: %s (%s) %s%s"
              % (cand, label, "usable" if ok else "not usable: " + err,
                 "  [same sound card as the microphone]" if same else ""))
        if not ok:
            continue
        if same:
            print("  !! this sound card is also the one the microphone is on: both directions share one I2S clock on the codec,")
            print("     so they can only use the same sample rate (the microphone is fixed at %d Hz, and A2DP's 44.1/48kHz fights with it)"
                  % SAMPLE_RATE)
            print("     workarounds (pick one of the two, try each once):")
            print("       HEADPHONE_CAPTURE_RATE=44100   capture the microphone at 44.1kHz (playback is preserved)")
            print("       HEADPHONE_HAT_MODE=1           drop the downlink to 16kHz mono")
            print("     the zero-trade-off option: plug the headphones into another sound card (the Pi's onboard 3.5mm / USB),")
            print("     or run sudo python3 %s --find-output and let it confirm which card the headphones are plugged into"
                  % os.path.basename(__file__))
        else:
            print("  -> not the same sound card as the microphone: the two directions do not affect each other (recommended)")
        return cand, label
    # 一张都打不开:给出板载音频没启用的可能
    names = [l for _c, _d, l in probe_playback_devices()]
    print("  playback devices in aplay -l: %s" % (", ".join(names) if names else "(none at all)"))
    if not names:
        print("  !! no playback device at all: onboard audio may be turned off. To get sound out of the Pi's onboard 3.5mm")
        print("     jack, make sure /boot/firmware/config.txt contains dtparam=audio=on")
        print("     (or /boot/config.txt depending on the OS version), then reboot and try again")
    return None, "no usable playback device"
...
def read_playback_mixer(playback_device):
    ...
    """
    if shutil.which("amixer") is None:
        print("  (amixer is missing, cannot read the mixer state)")
        return
    ...
    if not found:
        ...
        print(f"  mixer controls available on sound card {card}: {', '.join(names) if names else '(cannot read)'}")
...
def dump_all_mixer_controls(playback_device, limit=40):
    ...
    card = _playback_card(playback_device)
    names = list_mixer_control_names(card)
    print("-- all mixer controls of sound card %s (read-only; %d in total) --" % (card, len(names)))
    if not names:
        print("  (cannot read control names: amixer -c %s scontrols)" % card)
        return
    for ctl in names[:limit]:
        ...
        for v in values:
            print("      %s" % v)
    if len(names) > limit:
        print("  ...(%d more not printed)" % (len(names) - limit))
...
def apply_playback_mixer(playback_device, cross_route=False, quiet=False):
    ...
    if done and not quiet:
        print("  -> headphone jack mixer set: %s" % ", ".join(done))
    if failed and not quiet:
        print(
            "  !! failed to set these mixer controls: %s (use amixer -c %s scontrols to see the control names)"
            % (", ".join(failed), card)
        )
    if not done and not failed and not quiet:
        names = list_mixer_control_names(card)
        print(
            "  -> no known playback mixer control found on sound card %s: %s"
            % (card, ", ".join(names) if names else "(cannot read control names)")
        )
        print("     if the headphones have no sound, use alsamixer -c %s to adjust the volume/unmute manually" % card)
...
def decide_hat_mode(playback_device, mic_device):
    ...
    if not same_sound_card(playback_device, mic_device):
        print("  -> the microphone (%s) and the headphone jack (%s) are not on the same sound card: the two directions do not share a clock,"
              " and do not affect each other" % (mic_device, playback_device))
        return False
    print("  !! the microphone and the headphone jack are on the same sound card (%s): capture and playback share one I2S clock on the codec,"
          " so the two directions can only use the same sample rate" % playback_device)
    print("     the microphone over HFP is fixed at %d Hz, while A2DP music is 44100/48000 Hz - using both at once"
          " makes them fight over the clock" % SAMPLE_RATE)
    if MIC_CAPTURE_RATE != SAMPLE_RATE:
        print("  -> HEADPHONE_CAPTURE_RATE=%d is set: the microphone captures at that rate and the uplink"
              " resamples to %d Hz internally" % (MIC_CAPTURE_RATE, SAMPLE_RATE))
        print("     (playback keeps its native sample rate: both directions then use the same rate, and this playback config is the one verified audible)")
        return False
    opt = HEADPHONE_HAT_MODE
    if opt in ("1", "on", "true", "yes"):
        tested = []
        for rate, ch, label in ((SAMPLE_RATE, 1, "mono"), (SAMPLE_RATE, 2, "stereo")):
            ok, _err = test_alsa_playback(playback_device, rate=rate, channels=ch)
            tested.append("%d Hz %s %s" % (rate, label, "usable" if ok else "not usable"))
            if ok:
                _HAT_OUT[:] = [rate, ch]
                print("  -> same-card compatibility mode (HEADPHONE_HAT_MODE=%s): headphone downlink unified to "
                      "%d Hz %s" % (opt, rate, label))
                print("     note: on this card %d Hz playback may be silent in practice (it opens successfully but cannot be heard),"
                      " if there is no sound, set it back to auto and try HEADPHONE_CAPTURE_RATE=44100"
                      % SAMPLE_RATE)
                print("     self-test: %s" % "; ".join(tested))
                return True
        print("  !! the headphone jack does not support %d Hz playback, so the rate cannot be unified with the capture rate; self-test: %s"
              % (SAMPLE_RATE, "; ".join(tested)))
        return False
    print("  -> default policy (HEADPHONE_HAT_MODE=auto): playback keeps its native sample rate (verified audible),")
    print("     playback is not sacrificed to make room; if a clock conflict squeezes it out during concurrent use, the log will explain.")
    print("     two optional workarounds (try each once, use --hp-test to hear whether there is any sound):")
    print("       HEADPHONE_HAT_MODE=1             drop the downlink to 16kHz mono (may be silent on this card)")
    print("       HEADPHONE_CAPTURE_RATE=44100     capture the microphone at 44.1kHz + resample the uplink")
    print("     the safest approach is still to give the two directions different sound cards: plug the headphones into the Pi's onboard 3.5mm or a USB")
    print("     sound card and point PLAYBACK_DEVICE=plughw:C,D at it (there is no clock conflict across sound cards)")
    return False
...
def bluealsa_pcm_volume_note(device, profile):
    ...
    note = "  volume=%s" % ("unreadable" if vol is None else "%d/127" % vol)
    note += "  SoftVolume=%s" % _soft_volume_text(soft)
    if soft is True and (vol is None or vol == 0):
        note += "  !! soft volume scales the samples down to silence (the service raises it automatically while running)"
    return note
...
def _soft_volume_text(value):
    return {True: "true", False: "false", None: "unreadable"}[value]
...
STRIDE = 16   # take one sample out of every 16 for the peak: saves CPU, and a
              # 5 second window is plenty to spot silence
SILENCE_DB = -60.0   # below this the stream is essentially digital silence
LOW_DB = -25.0       # below this the level is clearly too low
# Notification tones injected into this stream (see the parent program's
# play_notice): the jack is already held by this pipeline, so a separate aplay
# could not use it. Format: name -> [(frequency Hz, duration ms), ...]
NOTICE_TONES = __NOTICE_TONES__
NOTICE_FILE = __NOTICE_FILE__
NOTICE_GAP_MS = 60
NOTICE_GAIN = 12000     # tone amplitude in S16 units (well below full scale)

# Pending notification tone: samples still to be mixed in (None = nothing due).
_notice = [None, 0]


def build_notice(spec):
    """Render a notification tone as interleaved S16 samples for CHANNELS."""
    out = array.array("h")
    for freq, ms in spec:
        n = int(RATE * ms / 1000.0)
        fade = max(1, int(RATE * 0.008))    # 8 ms fade: no click at either end
        for i in range(n):
            env = 1.0
            if i < fade:
                env = i / fade
                ...
                env = (n - i) / fade
            v = int(NOTICE_GAIN * env * math.sin(2.0 * math.pi * freq * i / RATE))
            for _ in range(CHANNELS):
                out.append(v)
        out.extend(array.array("h", [0] * (int(RATE * NOTICE_GAP_MS / 1000.0)
                                         * CHANNELS)))
    return out


def poll_notice():
    """Pick up a notification tone requested by the main program."""
    try:
        ...
        return
    spec = NOTICE_TONES.get(name)
    if spec:
        _notice[0] = build_notice(spec)
        _notice[1] = 0


def mix_notice(data):
    """Mix the pending notification tone into a block of forwarded audio."""
    tone = _notice[0]
    if tone is None or not data:
        return data
    body = array.array("h")
    body.frombytes(data[: len(data) - (len(data) % 2)])
    pos = _notice[1]
    n = min(len(body), len(tone) - pos)
    for i in range(n):
        v = body[i] + tone[pos + i]
        body[i] = 32767 if v > 32767 else (-32768 if v < -32768 else v)
    pos += n
    if pos >= len(tone):
        _notice[0] = None
        _notice[1] = 0
        ...
        _notice[1] = pos
    return body.tobytes() + data[len(body) * 2:]
...
def main():
    ...
    last = time.time()
    last_beat = last
    total = 0             # bytes forwarded so far (printed in the heartbeat:
                          # if it stops growing the pipeline is stuck)
    state = None          # last level class reported; a change is reported at once

    def report(force=False):
        # Report the peak seen since the previous report. A class change is
        # reported immediately, and every HEARTBEAT_SECS a heartbeat is forced:
        # without it "the pipeline has data but the jack is silent" and "the
        # pipeline is stuck" look identical in the log (that is where the last
        # debugging round got stuck).
        nonlocal peak, last, state, total
        cur, db = classify(peak)
        if cur != state or force:
            # The "level=" tag is parsed by the parent program - keep it as is.
            if cur == "silence":
                print("[downlink] %d Hz %d ch level=silence peak %s dBFS "
                      "forwarded %d bytes: there is almost no audio amplitude in "
                      "the stream - the audio was scaled to 0 before it reached "
                      "this machine, or the PC volume is 0/muted"
                      % (RATE, CHANNELS, "%.1f" % db if db is not None else "-inf",
                         total),
                      file=sys.stderr, flush=True)
            elif cur == "low":
                print("[downlink] %d Hz %d ch level=low peak %.1f dBFS forwarded "
                      "%d bytes" % (RATE, CHANNELS, db, total),
                      file=sys.stderr, flush=True)
            else:
                print("[downlink] %d Hz %d ch level=ok peak %.1f dBFS (normal) "
                      "forwarded %d bytes" % (RATE, CHANNELS, db, total),
                      file=sys.stderr, flush=True)
            state = cur
        peak = 0
        last = time.time()
    ...
    while True:
        # Check for a notification tone first, so one that arrives while the
        # downlink is streaming rides along with the very next block.
        poll_notice()
        # Poll stdin with a timeout so a tone is still played promptly when the
        # downlink happens to be idle (with a blocking read it would wait for
        # audio that may never come).
        try:
            ready = bool(select.select([stdin], [], [], 0.2)[0])
        except (OSError, ValueError):
            ready = True             # no select here: blocking read is all we have
        data = b""
        if ready:
            try:
            ...
            if not data:
                report(force=True)   # final report: shows if it ended silent
                break
        if not data:
            if _notice[0] is not None:
                # Downlink idle but a tone is due: push the whole tone out now,
                # in back-to-back chunks (waiting for more input between chunks
                # would break it up into clicks).
                while _notice[0] is not None:
                    left = len(_notice[0]) - _notice[1]
                    stdout.write(mix_notice(
                        array.array("h", [0] * min(left, 4096)).tobytes()))
                    stdout.flush()
            continue
        stdout.write(mix_notice(data))
        stdout.flush()
        total += len(data)
        # Level statistics come from the downlink audio only, never from the
        # injected tone (a tone must not mask a "silent stream" condition).
        n = len(data) // 2
...
def write_downlink_meter_script(path, rate, channels):
    ...
    path.write_text(
        ...
        # 提示音音型表与请求文件路径: 电平表用它把提示音混进下行音频，
        # 音型只有一个来源 (NOTICE_TONES)，两条播放路径的波形因此一致
        # (用 repr 注入: 路径里若有反斜杠也不会破坏脚本的字符串字面量)
        .replace("__NOTICE_TONES__", repr(NOTICE_TONES))
        .replace("__NOTICE_FILE__", repr(NOTICE_TONE_FILE))
    )
...
def main():
    try:
        ...
        print("[hp-mix] no numpy, falling back to decimation resampling", file=sys.stderr, flush=True)
...
def build_downlink_player(device, playback_device, python=None, prefer=None,
        ...
        if not rate:
            ...
            if not rate:
                ...
                if err and err != _LAST_DOWNLINK_ERR[0]:
                    ...
                    print("  !! cannot open the A2DP downlink PCM (tried both 44.1k/48k): %s" % err)
                    print("     it will be retried automatically once the PC starts playing")
                return None
        if not channels:
            channels = 2
    else:
    ...
    if need_mix:
        try:
            ...
            print("  !! cannot write the downlink mix script (%s): skipping sample rate unification this time" % exc)
            out_rate, out_channels = rate, channels
    rec_cmd = [
        "arecord",
        "-t",
...
def start_downlink_player(player, lines):
    ...
    # 丢掉上一次会话残留的提示音请求 (没有电平表阶段消费它时会留下)，
    # 否则它会在新管道启动后被当成一次新请求播出来
    try:
        os.unlink(NOTICE_TONE_FILE)
    except OSError:
        pass
    ...
    # 标记"下行管道在跑": 提示音据此改走"混入下行流"这条路 (见 play_notice)
    _DOWNLINK_UP.set()
    return procs
...
def meter_bytes(lines):
    ...
    for line in reversed(lines):
        # Only the machine tag "bytes=" that the meter prints is matched here:
        # changing the log wording can never silently break this again.
        m = re.search(r"forwarded (\d+) bytes", line)
        if m:
...
def downlink_level_state(lines):
    ...
    """
    for line in reversed(lines):
        # 只认电平表打的机器标签"level=": 文案改成英文/以后再改措辞都不会让
        # 这里的判断失效 (原先靠匹配中文句子，改文案就会静默失配)
        if "level=" not in line:
            continue
        if "level=silence" in line:
            return "silence"
        if "level=low" in line:
            return "low"
        return "ok"
    return None
...
def print_a2dp_silent_hint():
    ...
    print("  !! the downlink level is almost digital silence: the audio was scaled to ~0 before it reached the Pi,")
    print("     the problem is not the headphone jack/mixer (the local test tone is fine), check in this order:")
    print("     A. PC side (most common):")
    print("        * Sound settings -> Output -> open this device and check whether its volume slider is pulled down very low;")
    print("        * Control Panel -> Sound -> 'Communications' tab: by default, when it detects communication activity it")
    print("          'reduces the volume of other sounds by 80%' or even mutes them - and this machine is also a microphone")
    print("          (a communications device), so as soon as the microphone is on the PC ducks the music. Change it to")
    print("          'Do nothing'.")
    print("     B. Raspberry Pi side (A2DP volume model): currently SoftVolume=false and the bluealsa")
    print("        startup arguments have no --a2dp-volume - an undefined half-state, in which samples may be scaled by the AVRCP")
    print("        volume (0). Pick one of the two proper approaches:")
    print("        * Let this machine control the volume itself:")
    print("          bluealsa-cli soft-volume <A2DP PCM path> true")
    print("          bluealsa-cli volume <A2DP PCM path> 127")
    print("        * Let the PC slider control the volume: add a drop-in to the service")
    print("          Environment=BLUEALSA_EXTRA_ARGS=--a2dp-volume")
    print("        (for the A2DP PCM path, see the output of sudo python3 %s --audio-info)"
          % os.path.basename(__file__))
...
def force_local_a2dp_volume(device):
    ...
    if not path:
        return False
    print("  -> Switching the A2DP downlink to local volume control: soft-volume true + volume %d" % A2DP_MIN_VOLUME)
    ...
    soft = read_bluealsa_soft_volume(path)
    vol = read_bluealsa_volume(path)
    print("     Now: SoftVolume=%s volume=%s (if the headphones are still silent, watch the level meter: if the level does not change,"
          " that means the amplitude is being suppressed on the PC side)"
          % (_soft_volume_text(soft), "unreadable" if vol is None else "%d/127" % vol))
    return True
...
#     才看一次，而且判定"电平近乎静音"还要连着 3 次 (≈90 seconds)才动手——开麦克风
...
ROUTE_LABELS = {
    "a2dp": "A2DP music route (the '...  Stereo' entry in Windows, 44.1/48kHz stereo)",
    "sco": "Hands-Free downlink (the '...  Hands-Free AG Audio' entry in Windows, 16kHz mono)",
}


def describe_route(profile):
    """下行通路的人话名字 (日志/提示里用)。"""
    return ROUTE_LABELS.get(profile, profile or "unknown route")
...
def headphone_monitor(device, stop_event, playback_device=None, hat_mode=False,
    ...
    if not HEADPHONE_ENABLE:
        print("  -> HEADPHONE_ENABLE=0: skipping Bluetooth headphone (speaker) output")
        return
    for tool in ("arecord", "aplay"):
        if shutil.which(tool) is None:
            print("  !! Missing %s (apt install alsa-utils); Bluetooth headphone output unavailable" % tool)
            print("     Microphone function is unaffected")
    ...
    if playback_device is None:
        ...
        if playback_device is None:
            print(f"  !! No headphone jack playback device found ({why}): Bluetooth headphone function skipped")
            write_headphone_status(state="unavailable", reason=why)
            return
        apply_playback_mixer(playback_device, cross_route=cross_route)

    print("  -> Bluetooth headphone output is ready (select this machine as the output device on the PC and you will hear sound)")
    print("     Playback method: arecord <downlink PCM> | aplay -D %s%s"
          % (playback_device,
             " (HAT compatibility mode: downlink unified to %d Hz mono, same rate as the microphone)"
    ...
    write_headphone_status(
        ...
        hat_mode="yes (downlink unified to %d Hz %d ch, same rate as the microphone)"
    ...
    playback_notice_done = False   # "放音可用"提示音每会话只播一次
    _PLAYBACK_DEVICE[0] = playback_device   # 提示音直接用这个设备
    ...
    def switch_downlink_route(reason, target=None, force=False):
        ...
        if prof is None:
            print("  -> The other downlink route does not exist right now, cannot switch yet (waiting for the PC to establish it)")
            return False
        if pcm is None:
        ...
        if not force:
            if not pcm:
                print("  -> Cannot get the PCM name of the other downlink route (the list tool did not report it), not switching for now")
                return False
            now = time.time()
            ...
            if note != probe_note:
                probe_note = note
                if live:
                    print("  -> Liveness probe result: audio data was read on %s" % describe_route(prof))
                else:
                    print("  -> Liveness probe result: no audio data can be read on %s either (the PC is not sending audio to that one),"
                          " not switching for now" % describe_route(prof))
            if not live:
                return False
        if route_switches >= MAX_ROUTE_SWITCHES:
            if not cap_noted:
                cap_noted = True
                print("  -> Switched %d times already, no more automatic switching so it does not hop back and forth"
                      " (you can pin the route manually with HEADPHONE_PROFILE)" % route_switches)
            return False
        print("  -> Downlink route switch (%s): %s -> %s"
        ...
        while not stop_event.is_set():
            if time.time() - last_bt_check >= 5:
                ...
                if not is_device_connected(device):
                    print("  -> Headphone output: Bluetooth connection lost, stopping playback")
                    break
            ...
            if time.time() - last_a2dp_check >= A2DP_CHECK_SECS:
                ...
                if a2dp_state is None:
                    a2dp_state = active
                    if active:
                        print("  -> A2DP music channel detected (the PC is streaming audio over)")
                    ...
                    a2dp_state = active
                    print(
                        "  -> The PC %s the music channel (A2DP)"
                        % ("started using" if active else "stopped")
                    )
                    if active:
                ...
                if not active and not no_a2dp_hint and _A2DP_REGISTERED[0]:
                    no_a2dp_hint = True
                    print("  !! The PC is connected, but the A2DP music channel has not been established yet:")
                    print("     In Windows, only when it has cached this machine's Audio Sink service will")
                    print("     'Sound settings -> Output' show 'Headphones (device name Stereo)'.")
                    print("     If the output list only shows a microphone / no such device, the service list used at pairing time")
                    print("     is stale: delete the device in Windows and pair again, or run on the Raspberry Pi")
                    print("     sudo python3 %s --forget so that the PC pairs again." % os.path.basename(__file__))
                    print("     For troubleshooting: sudo python3 %s --audio-info" % os.path.basename(__file__))
            ...
            if active_now != uplink_seen:
                ...
                if active_now:
                    print("  -> Microphone uplink started: rechecking whether the headphone jack mixer was"
                          " reset to default by the sound card driver")
            ...
            if (playback_device and _HP_MIXER_APPLIED
                ...
                if drift:
                    print("  !! Headphone jack mixer was reset to default by the sound card driver (this happens when the capture stream is opened / the "
                          "codec is reconfigured, and that is exactly why there is no sound after starting the microphone):")
                    for line in drift[:8]:
                        print("     %s" % line)
                    if mixer_refix < 5:
                        mixer_refix += 1
                        ...
                        print("  -> Headphone jack mixer re-applied (attempt %d)" % mixer_refix)
                    elif not mixer_giveup_noted:
                        mixer_giveup_noted = True
                        print("  !! The mixer keeps being reset (fighting back and forth with the driver), no longer re-applying it automatically.")
                        print("     You can verify manually with alsamixer -c %s, or plug the headphones into"
                              " another sound card (PLAYBACK_DEVICE=plughw:C,D)"
                              % _playback_card(playback_device))
            # 下行通路健康检查 (每 DOWNLINK_CHECK_SECS seconds):先看这条通路上
            # 到底有没有数据在流，再看电平是否被缩放到近乎 0
            if procs and time.time() - last_downlink_check >= DOWNLINK_CHECK_SECS:
                last_downlink_check = time.time()
                uptime = time.time() - started
                got_data = any("level=" in line for line in lines)
                ...
                if not progressed and uptime >= DOWNLINK_STALL_SECS:
                    ...
                    if not stall_noted:
                        stall_noted = True
                        print("  !! Headphone output: no new data on %s (running for %.0f seconds,"
                              " cumulative bytes %s)"
                              % (describe_route(cur_profile), uptime,
                                 "unknown" if now_bytes is None else now_bytes))
                        print("     A route being up does not mean the PC is sending audio to it; it may also be that the downstream"
                              " sound card has stopped consuming (sample rate conflict on the same card)")
                        print("     Probing the other route...")
                    if switch_downlink_route("no new data on the current route"):
                        continue
                elif level in ("silence", "low"):
                    silent_strikes += 1
                    print(
                        "  !! Downlink level %s (%d times in a row): the audio is scaled to ~0 before"
                        " it even reaches the headphone jack"
                        % ("fully silent" if level == "silence" else "too low", silent_strikes)
                    )
                    if silent_strikes == 2 and not volume_fix_done:
                        volume_fix_done = True
                        force_local_a2dp_volume(device)
                        print("     If the level changes along with it -> it is a volume model problem on the Raspberry Pi side;")
                        print("     If the level does not change at all -> the amplitude is being suppressed on the PC side (see the guidance below)")
                    elif silent_strikes >= 3:
                        # 先探活另一条通路:有数据就切过去 (开麦克风后电脑常把
                        # 输出送到另一条)；另一条也没数据就打印电脑侧排查指引
                        if switch_downlink_route("the level on this route is almost 0"):
                            continue
                    ...
                    if silent_strikes:
                        print("  -> Downlink level is back to normal (the peak is now visible), no longer counted as silent")
                    silent_strikes = 0
                    silent_hint_shown = False
            ...
            if not procs:
                ...
                if player is None:
                    if not no_pcm_noted:
                        no_pcm_noted = True
                        print("  -> Headphone output: no usable downlink PCM on this machine right now (the PC has not")
                        print("     selected this machine as the output device, or has not set up an A2DP/Hands-Free route yet)")
                    restart = 0
                    stop_event.wait(3)
                    continue
                lines = []
                # 先拿提示音锁: 正在播的提示音占着声卡时，直接启动播放器会
                # EBUSY (提示音最多 1 秒，等它播完再起，比让播放器启动失败好)
                with _NOTICE_LOCK:
                ...
                last_downlink_check = time.time()
                print(
                    "  -> Headphone output player started: %s downlink, %d Hz %d ch, PID %s"
                    % (
                        player["profile"],
                        player["rate"],
                        player["channels"],
                ...
                if not playback_notice_done:
                    # 放音通路第一次真正跑起来: 播提示音报"放音可用"
                    playback_notice_done = True
                    play_notice("playback")
                stop_event.wait(2)
                continue
            dead = [
            ...
            if not dead:
                ...
                continue
            print(
                "  !! Headphone output player exited (%d/%d processes: %s)"
            ...
            if restart >= 2 and switch_downlink_route("the player keeps failing to start", force=True):
                restart = 0
                continue
            if restart > MAX_RESTARTS:
                ...
                print("  !! Headphone output failed to start %d times in a row: retrying in 30 seconds"
                      " (not giving up on this session, microphone function is unaffected)" % restart)
...
def ensure_a2dp_audible(device):
    ...
    vol = read_bluealsa_volume(path)
    soft = read_bluealsa_soft_volume(path)
    vol_text = "unreadable" if vol is None else "%d/127" % vol
    note = "volume=%s softvol=%s" % (vol_text, _soft_volume_text(soft))
    if vol is None:
        print("  -> The A2DP downlink volume attribute is unreadable (SoftVolume=%s)" % _soft_volume_text(soft))
        print("     Not touching the stream that is playing (in practice, changing the soft volume makes the stream go silent);")
        print("     If there really is no sound, you can try the commands listed in --audio-info by hand")
        return note
    if vol > 0:
        print("  -> A2DP downlink volume=%s SoftVolume=%s (normal)"
              % (vol_text, _soft_volume_text(soft)))
        return note
    print("  !! A2DP downlink volume=0: the whole stream will be scaled to silence, raising it to %d" % A2DP_MIN_VOLUME)
    res = run(
        f"bluealsa-cli volume {path} {A2DP_MIN_VOLUME}",
        check=False,
        timeout=8,
    ...
    if res and res.returncode == 0 and vol2:
        print("     Set to %d/127 (the volume is controlled by the local mixer from now on)" % vol2)
        return "volume=%d/127 softvol=%s" % (vol2, _soft_volume_text(soft))
    print("     The setting did not take effect (the volume is still unreadable); not changing SoftVolume again, to avoid interrupting the stream that is playing")
    return note


def make_notice_tone_raw(spec, rate=NOTICE_TONE_RATE, channels=2, gain=12000):
    """把提示音音型渲染成 S16_LE 原始样本 (头尾各 8ms 淡入淡出，避免爆音)。

    spec 形如 [(频率 Hz, 时长 ms), ...]，各音之间插一段静音。电平表阶段注入
    的波形与这里完全一致 (同一张 NOTICE_TONES 表)，所以两条播放路径听感相同。
    """
    frames = bytearray()
    for freq, ms in spec:
        n = int(rate * ms / 1000.0)
        ...
        for i in range(n):
            ...
            if i < fade:
                ...
                env = (n - i) / fade
            v = int(gain * env * math.sin(2.0 * math.pi * freq * i / rate))
            for _ in range(channels):
                frames += int(v).to_bytes(2, "little", signed=True)
        frames += b"\x00\x00" * (int(rate * NOTICE_TONE_GAP_MS / 1000.0) * channels)
    return bytes(frames)


def _play_notice_direct(dev, kind):
    """直接用 aplay 播一段提示音 (下行管道没在跑、声卡空闲时才走这条路)。"""
    path = "/tmp/bt_notice_tone.raw"
    try:
        with open(path, "wb") as f:
            f.write(make_notice_tone_raw(NOTICE_TONES[kind]))
    except OSError:
        return
    if not _NOTICE_LOCK.acquire(timeout=2.0):
        return                      # 声卡被别的东西长期占着，这次就算了
    try:
        run(
            f"timeout 6 aplay -t raw -D '{dev}' -f {FORMAT} "
            f"-r {NOTICE_TONE_RATE} -c 2 '{path}'",
            check=False,
            timeout=10,
            verbose=False,
        )
...
def play_notice(kind):
    """播一次提示音 (kind: "bt" / "denoise" / "playback")，不阻塞调用方。

    难点在于放音设备通常正被下行播放管道独占 (aplay 会一直占着那张卡)，这时
    再起一个 aplay 只会 EBUSY。所以分两条路走:
      * 下行管道在跑 → 把音型名写进 NOTICE_TONE_FILE，由管道中间的电平表
        阶段把波形混进正在播放的音频里——提示音走的就是用户正在听的那条路，
        也不会有第二个进程去抢声卡；
      * 下行管道没跑 (刚连上、电脑还没开始放音) → 声卡是空闲的，直接起一个
        短 aplay 播完就退出。
    两条路都是 best-effort: 失败只影响提示音本身，绝不影响音频转发。
    """
    if not NOTICE_TONE_ENABLE or kind not in NOTICE_TONES:
        return
    if _DOWNLINK_UP.is_set():
        try:
            Path(NOTICE_TONE_FILE).write_text(kind)
            print("  -> Notification tone: %s (mixed into the downlink stream)" % kind)
            return
        except OSError as exc:
            print("  !! Could not write the notification tone request (%s)" % exc)
            return
    dev = PLAYBACK_DEVICE or _PLAYBACK_DEVICE[0]
    if dev is None:
        dev, _why = select_playback_device(MIC_DEVICE or None)
        _PLAYBACK_DEVICE[0] = dev
    if dev is None:
        return                      # 没有播放设备: 提示音自然无从谈起
    print("  -> Notification tone: %s (played on %s)" % (kind, dev))
    threading.Thread(target=_play_notice_direct, args=(dev, kind), daemon=True).start()
...
def headphone_test():
    ...
    """
    print_status("Headphone jack step-by-step self-test (--hp-test)")
    print("  Tip: this self-test needs exclusive access to the sound card. If the bt-mic service is running and a device is connected, first run")
    print("        sudo systemctl stop bt-mic and then run this command (systemctl start when done) -")
    print("        otherwise the service's capture/playback streams and the tests here will fight each other over the sound card, and the results will be wrong")
    if shutil.which("aplay") is None or shutil.which("arecord") is None:
        print("  !! Missing aplay/arecord (apt install alsa-utils)")
        return
    mic_device, mic_channels = select_and_verify_mic()
    dev, why = select_playback_device(mic_device)
    if dev is None:
        print(f"  !! No usable playback device found: {why}")
        return
    print(f"  -> Playback device: {dev} ({why})")
    # 第 0 步:换过耳机插孔之后"没声音"最常见的原因就是声音还送给原来那张卡。
    # 先把所有播放声卡列出来 (标出与麦克风同卡的, 需要的话直接逐卡试听并保存
    list_playback_cards(mic_device)
    print("  If the headphones are already plugged into the other jack (HAT 3.5mm <-> Raspberry Pi onboard 3.5mm),")
    print("  then the currently selected %s is not the card the headphones are on." % dev)
    try:
        ans = input("  Play a test tone on each sound card to find out which one the headphones are plugged into? (y/N): ").strip().lower()
    except EOFError:
        ans = "n"
        print("  (non-interactive environment: skipping the per-card listening test; you can run sudo python3 %s --find-output separately)"
              % os.path.basename(__file__))
    if ans.startswith("y"):
        find_output_device()
        return
    ...
    if mic_device:
        print(f"  -> Microphone: {mic_device} ({mic_channels} channels, capture {MIC_CAPTURE_RATE} Hz)")
        same = same_sound_card(dev, mic_device)
        print("  -> The microphone and the headphone jack %s"
              % ("**are on the same sound card** (they share the I2S clock, so only one sample rate is possible)" if same
                 else "are two different sound cards (they do not affect each other)"))
    ...
    print("-- Setting the headphone jack mixer based on the above (including cross routing)--")
    apply_playback_mixer(dev, cross_route=cross_route)

    print_status("Step 1: play 1kHz once for left/right/both channels - listen carefully to which side sounds")
    tone = "/tmp/bt_hp_test.wav"
    for side, label, tip in (
        ("l", "only the left channel has signal", "only the left ear should sound"),
        ("r", "only the right channel has signal", "only the right ear should sound"),
        (None, "both left and right channels have signal", "both ears should sound"),
    ):
        try:
            write_test_tone_wav(tone, 2.0, 1000.0, 44100, 2, side=side)
        except OSError as exc:
            print(f"  !! Failed to generate the test tone ({exc})")
            return
        print(f"  -> [LISTEN] {label} ({tip})...")
        res = run(f"aplay -D '{dev}' '{tone}'", check=False, timeout=25, verbose=False)
        if res is not None and res.returncode != 0:
            err = " ".join(((res.stdout or "") + (res.stderr or "")).split())
            print(f"     !! aplay failed: {err[-200:]}")
        time.sleep(0.6)
    print("  ★ Remember which ear you hear in each of these three steps (e.g. only the left ear in all three)")

    print_status("Step 2: objective behavior of the headphone jack in each format (no ears needed)")
    print("  Criterion: underrun=0 and elapsed time ≈ playback duration -> the hardware is running at the declared sample rate for that format;")
    print("        elapsed time clearly too long -> the hardware's actual sample rate does not match what we declared (same-card clock conflict)")
    results = []
    for rate, ch in ((44100, 2), (SAMPLE_RATE, 1), (48000, 2)):
        r = play_raw_measured(dev, rate, ch, seconds=2.0)
        results.append(r)
        print("  %5d Hz %d ch -> exit code %s, underrun %d times, elapsed %.1fs (expected 2.0s)%s"
              % (rate, ch, r["rc"], r["underrun"], r["elapsed"],
                 "  " + r["err"] if r["err"] else ""))

    print_status("Step 3: microphone capture level (each sample rate, no ears needed)")
    if not mic_device:
        print("  (no usable microphone found, skipping)")
    else:
        for cap_rate in (SAMPLE_RATE, 44100, 48000):
            lvl = measure_capture_level(mic_device, cap_rate, mic_channels, seconds=1)
            print("  Capture %5d Hz -> %s" % (cap_rate, lvl or "cannot open / cannot read"))
        print("  Note: only sample rates that capture a normal level (not digital silence) are usable;")
        print("        If 44100 Hz can be captured, we recommend HEADPHONE_CAPTURE_RATE=44100:")
        print("        playback stays 44.1 kHz stereo and the microphone captures at the same rate, so neither direction goes silent")

    print_status("Step 4: record and play at the same time (play audio while the microphone is really running)")
    print("  Each round starts recording first (-d 30 limits the time and makes sure the device is released at the end), then plays 2 seconds of test tone;")
    print("  A 'Device or resource busy' on the recording side means capture cannot open (the device is occupied),")
    print("  more underruns or a longer elapsed time on the playback side means the two directions are fighting over the same clock")
    if not mic_device:
        print("  (no usable microphone found, skipping)")
    else:
            ...
            for rate, ch in ((rate, ch) for rate, ch in ((44100, 2), (SAMPLE_RATE, 1))):
                ...
                for line in cap_out.splitlines():
                    ...
                    if "busy" in low or "error" in low or "overrun" in low:
                        cap_err = "; recording side: " + line.strip()[:110]
                        break
                print("  Capture %5d Hz + playback %5d Hz %d ch -> playback underrun %d times, "
                      "elapsed %.1fs%s%s"
                      % (cap_rate, rate, ch, r["underrun"], r["elapsed"],
                         "" if alive else "; recording failed to start",
                         cap_err))
                time.sleep(0.5)
    print_status("How to read the results / next steps")
    print("  1) In step 1, 'only the left ear' or 'only the right ear' with all three steps the same -> the card's other")
    print("     channel is not wired (hardware). The software already folds the downlink to mono and enables HP cross routing,")
    print("     if only one side still sounds, the only option is to change the sound card: plug the headphones into the Pi's onboard 3.5 mm jack or a USB sound card,")
    print("     and specify it with PLAYBACK_DEVICE=plughw:C,D (see the card numbers with aplay -l)")
    print("  2) If the 'capture 16000 + playback 44100' line in step 4 has clearly more underruns than")
    print("     'capture 16000 + playback 16000', that confirms the same-card sample rate conflict. Try each fix once:")
    print("       HEADPHONE_CAPTURE_RATE=44100  (recommended: playback keeps 44.1 kHz stereo)")
    print("       HEADPHONE_HAT_MODE=1          (downlink drops to 16 kHz mono)")
    print("  3) If in step 2 the 16 kHz line has underrun=0 but you hear nothing, while 44.1 kHz is audible")
    print("     -> 16 kHz playback on this card is unreliable, so be sure to use the HEADPHONE_CAPTURE_RATE route")
    print("  4) There is a pause between each combination so you can tell them apart; the hp-play underruns in the log can also")
    print("     be compared afterwards (when the service runs the stage names are hp-*, no longer confused with the microphone pipeline)")
...
def list_playback_cards(mic_device=None):
    ...
    if shutil.which("aplay") is None:
        print("  !! aplay is missing (apt install alsa-utils)")
        return []
    mic_card = _playback_card(mic_device) if mic_device else None
    cards = probe_playback_devices()
    if not cards:
        print("  aplay -l lists no playback device at all: onboard audio may be disabled")
        print("  To get sound from the onboard 3.5 mm jack, make sure /boot/firmware/config.txt has dtparam=audio=on")
        return []
    print("-- all playback sound cards (the sound must go to whichever card's jack the headphones are plugged into) --")
    for idx, (card, dev, label) in enumerate(cards, 1):
        cand = "plughw:%d,%d" % (card, dev)
        mark = "  [same sound card as the microphone]" if (mic_card and str(card) == mic_card) else ""
...
def find_output_device():
    ...
    """
    print_status("Find which sound card the headphones are plugged into (--find-output)")
    print("  Tip: run sudo systemctl stop bt-mic first, otherwise the service holds the sound card and the test tone may be wrong")
    mic_device = None
    ...
    if mic_device:
        print("  Microphone device: %s (cards on the same sound card as it are marked below)" % mic_device)
    ...
    try:
        ...
        print("  !! Failed to generate the test tone (%s)" % exc)
        return
    chosen = None
    for idx, (card, dev, label) in enumerate(cards, 1):
        cand = "plughw:%d,%d" % (card, dev)
        ok, err = test_alsa_playback(cand)
        if not ok:
            print("  [%d] %s cannot be opened (%s), skipping" % (idx, cand, err))
            continue
        print("  [%d] Playing 2.5 seconds of 1 kHz through %s (%s) [LISTEN]..." % (idx, cand, label))
        ...
        run(f"aplay -D '{cand}' /tmp/bt_find_output.wav", check=False, timeout=25,
            verbose=False)
        try:
            ans = input("      Did you hear it? (y=this is the one / Enter=no): ").strip().lower()
        except EOFError:
            print("      (non-interactive environment, skipping the prompt: note the card that produced sound and specify it with "
                  "PLAYBACK_DEVICE=)")
            continue
        if ans.startswith("y"):
            chosen = cand
            break
    if chosen is None:
        print("  -> No sound card was confirmed to produce sound. For reference:")
        print("     * Headphones in the Pi's onboard 3.5 mm jack -> need dtparam=audio=on (config.txt)")
        print("     * Headphones in the HAT's 3.5 mm jack -> that card is seeed/respeaker/voicecard")
        print("     * Volume/mute: in alsamixer -c <card number>, raise PCM and unmute")
        print("     * Full diagnostics: sudo python3 %s --audio-info" % os.path.basename(__file__))
        return
    print("  -> Confirmed to use: %s (%s)" % (chosen, dict((f"plughw:{c},{d}", l)
                                                for c, d, l in cards).get(chosen, "")))
    ok, info = save_config_key("PLAYBACK_DEVICE", chosen)
    if ok:
        print("  -> Wrote config %s: PLAYBACK_DEVICE=%s" % (info, chosen))
        print("     Takes effect: sudo systemctl restart bt-mic (or rerun this program in the foreground)")
        try:
        ...
        print("  !! Failed to write the config (%s): please add the line by hand" % info)
        print("     PLAYBACK_DEVICE=%s   to %s" % (chosen, CONFIG_FILE))
...
def play_test_tone():
    ...
    """
    print("=== Headphone jack (3.5 mm) test ===")
    caps = probe_input_devices()
    ...
    if dev is None:
        print(f"  !! No usable playback device found: {why}")
        return
    print(f"  -> Using playback device: {dev} ({why})")
    ...
    if same_card_mic:
        print("  -> This card is also the microphone's sound card: HP cross routing is enabled (both outputs produce sound);")
        print("     to check whether the left/right channels and record-while-play work normally, run sudo python3 %s --hp-test"
              % os.path.basename(__file__))
    path = "/tmp/bt_headphone_test.wav"
    try:
        write_test_tone_wav(path)
    except OSError as exc:
        print(f"  !! Failed to generate the test tone ({exc}), please check whether /tmp is writable")
        return
    print(f"  -> Playing a 2 second 1 kHz test tone to {dev} (put on / plug in your headphones)...")
    run(f"aplay -D '{dev}' '{path}'", check=False, timeout=20)
    print("  -> Test finished: hearing the tone means the headphone jack and the mixer are working;")
    print("     if silent, make sure the headphones are in the reSpeaker HAT's 3.5 mm jack and pushed in firmly,")
    print("     and try raising the volume (rerun this command after HEADPHONE_VOLUME=100%)")
...
def print_audio_info():
    ...
    并给出结论与处理办法。
    """
    print("=== Bluetooth audio output (headphones/speaker) diagnostics ===")
    print("  Build version: %s" % BUILD_ID)
    print("  (If the version shown here is older than the script on disk, the service is still running the old process:")
    print("    run sudo systemctl restart bt-mic and then run the diagnostics again)")
    ...
    print("  Class should be 0x240404 (audio device / wearable headset): Windows uses it to decide")
    print("  which category the Raspberry Pi belongs to, and therefore which endpoints to create (microphone / headphone output)")

    print("-- BlueALSA startup arguments --")
    exec_line = "(no override file found, bluealsa is probably using the system default arguments)"
    try:
    ...
    print(
        "  Contains A2DP Sink: %s (without it the PC will never see the 'headphones/stereo' output entry)"
        % ("yes" if has_a2dp_arg else "no")
    )
    st = run("systemctl is-active bluealsa", check=False, timeout=8, verbose=False)
    print(f"  Service status: {st.stdout.strip() if st else '?'}")
    print("  If A2DP registration fails, check: journalctl -u bluealsa -n 50 --no-pager")

    print("-- local SDP service records (these are what the PC sees when pairing) --")
    names = sdp_local_service_names()
    a2dp_sdp = sdp_has_a2dp_sink(names)
    hfp_sdp = sdp_has_handsfree(names)
    if names is None:
        print("  (sdptool unavailable or the query failed, cannot confirm)")
    else:
        for n in names:
            print(f"  - {n}")
        print(
            "  -> Handsfree/Headset (needed when used as a microphone): %s"
            % ("yes" if hfp_sdp else "missing")
        )
        print(
            "  -> Audio Sink / A2DP (needed as an output device): %s"
            % ("yes" if a2dp_sdp else "missing")
        )

    print("-- all sound cards (raw output of aplay -l / arecord -l) --")
    for cmd in ("aplay -l", "arecord -l"):
        print(f"  $ {cmd}")
        res = run(cmd, check=False, timeout=8, verbose=False)
        text = ((res.stdout or "") + (res.stderr or "")) if res else "(execution failed)"
        shown = False
        for line in text.splitlines():
            if line.strip():
                print(f"    {line.rstrip()}")
    ...
    print(f"  Playback devices: {len(pb)}(of which reSpeaker/wm8960 sound cards: {len(hat_pb)})")
    print(f"  Recording devices: {len(cap)}(of which reSpeaker/wm8960 sound cards: {len(hat_cap)})")
    if not hat_pb and hat_cap:
        print("  !! The reSpeaker sound card appears only in the recording list (no playback device): that card's playback")
        print("     route is not registered by the driver, so the HAT's 3.5 mm jack cannot play audio")
    elif not hat_pb and not hat_cap:
        print("  !! No seeed/respeaker/wm8960 sound card at all: the HAT driver is not loaded (or no HAT is connected),")
        print("     the 3.5 mm headphone jack on the HAT will not produce sound - only the Pi's onboard 3.5 mm jack will")

    print("-- headphone jack (output device) --")
    ...
    if dev:
        ...
        if mic_dev_hint and same_sound_card(dev, mic_dev_hint):
            print("  -> The headphone jack and microphone are on the same sound card (TX/RX share the I2S clock, so they can only use one")
            print("     sample rate): the service enables same-card compatibility mode, unifying the headphone downlink to %d Hz %d ch"
                  % (_HAT_OUT[0], _HAT_OUT[1]))
            print("     When both directions use this card, A2DP stereo drops to that sample rate (trading audio quality for coexistence)")
        else:
            print("  -> The headphone jack and microphone are not on the same sound card: the two directions do not affect each other, so no compatibility mode is needed")
    else:
        print(f"  No usable playback device found: {why}")

    print("-- Current downlink (playable) PCM --")
    devices = get_connected_devices()
    a2dp_now = False
    if not devices:
        ...
        if pcm_list:
            for p in pcm_list:
                ...
                print(f"      D-Bus path: {path}")
                print("      Manual reproduction (same playback method as inside the program, verifies the BlueALSA -> headphone jack segment):")
                ...
                if dev and same_sound_card(dev, mic_dev_hint):
                    ...
                    print("      ↑ But the microphone and headphone jack are on the same sound card (TX/RX share the I2S clock),")
                    print("        so in practice the service reduces the downlink to this last format before sending it to the headphone jack:")
                    print("        arecord -D '%s' -f %s -r %d -c %d -t raw "
                ...
                if prof == "a2dp":
                    print("      A2DP volume model (consult here when the downlink is scaled to ~0):")
                    print("        current: SoftVolume=%s, bluealsa startup args%s --a2dp-volume"
                          % (
                              _soft_volume_text(read_bluealsa_soft_volume(path)),
                              " include" if "--a2dp-volume" in BLUEALSA_EXTRA_ARGS else " do not include",
                          ))
                    print("        choose one of the two proper approaches (if it is currently false and there is no --a2dp-volume,")
                    print("        that is an undefined half-state and it may be scaled by AVRCP volume 0):")
                    print("        1) Control the volume on this machine (recommended):")
                    print("           bluealsa-cli soft-volume %s true" % path)
                    print("           bluealsa-cli volume %s %d" % (path, A2DP_MIN_VOLUME))
                    print("        2) Control the volume with the PC slider: add a drop-in for the service")
                    print("           Environment=BLUEALSA_EXTRA_ARGS=--a2dp-volume")
                    print("        also confirm on the PC side: (a) the volume slider for this device in the output list is not at the minimum;")
                    print("        (b) in Control Panel -> Sound -> 'Communications' tab, change it to 'Do nothing'")
                    print("         - the default setting, when it detects communications activity (we are acting as a microphone), turns other")
                    print("        sounds down by 80% or even silences them, which shows up as 'there is sound at first, then no sound'")
        else:
            print("  (no playable PCM: the PC has not yet chosen this machine as an output device)")
        print(f"  Connected devices: {', '.join(devices)}")

    print("-- Headphone output playback method and service status --")
    print("  playback method: arecord <downlink PCM> | aplay -D <headphone jack> (built into this program)")
    print("  bluealsa-aplay is not used: its options differ wildly between versions, and in testing some versions")
    print("  have no option to specify the output device, so passing -d exits immediately with `invalid option -- 'd'`")
    res = run(
    ...
    if running:
        print("  currently running downlink/forwarding processes:")
        for line in running:
            print(f"    {line}")
    else:
        print("  no downlink playback process right now - it appears only after the PC connects and this program starts it;")
        print("  if the service is not running (systemctl status bt-mic), the headphones will make no sound")
    if os.path.exists(HEADPHONE_STATUS_FILE):
        print("  status file %s:" % HEADPHONE_STATUS_FILE)
        try:
    ...
    keys = ("headphone", "music channel", "bluealsa-aplay", "A2DP", "player exit")
    hits = [
        l.strip()
        for l in ((res.stdout or "") if res else "").splitlines()
        # 大小写不敏感:日志文案会写成 "Headphone output ..." 这样的大写开头，
        # 区分大小写的匹配会漏掉正是要找的那几行
        if any(k.lower() in l.lower() for k in keys)
    ]
    if hits:
        print("  most recent bt-mic service log records related to headphone output:")
        for line in hits[-8:]:
            print(f"    {line}")

    print("-- Conclusion and what to do --")
    if not has_a2dp_arg:
        print("  1) bluealsa was started without A2DP Sink args -> an audio output entry can never appear on the PC.")
        print("     confirm bluez-alsa has A2DP support (apt install bluez-alsa-utils),")
        print("     then restart the service so this program registers again: systemctl restart bt-mic")
    elif a2dp_sdp is False:
        print("  1) there is no Audio Sink record in this machine's SDP -> A2DP did not register successfully,")
        print("     check journalctl -u bluealsa for the reason (profile name not supported, etc.)")
    else:
        print("  1) this machine's SDP already has an Audio Sink record (the Raspberry Pi side is fine)")
    if a2dp_now:
        print("  2) the PC is using the A2DP music channel (the Bluetooth link itself is fine). When you hear no")
        print("     sound, check these three places in order:")
        print("     a) the [headphone jack] section above: is the playback device it selected the jack you plugged your headphones into?")
        print("        the onboard jack shows bcm2835 Headphones; the HAT jack shows seeed/wm8960.")
        print("        choosing the wrong jack (e.g. headphones plugged into the HAT but the sound sent to the onboard jack) means silence.")
        print("     b) run sudo python3 %s --play-test: plays a 1kHz test tone directly on this machine."
              % os.path.basename(__file__))
        print("        sound = the headphone jack and mixer are fine, the problem is on the Bluetooth side; no sound = jack or mixer problem")
        print("     c) 'volume=0 / unreadable' for A2DP in the PCM line above: for A2DP BlueALSA turns on soft volume")
        print("        by default, so samples are scaled by that volume; 0 or unreadable means the whole stream is silent")
        print("        (the service automatically raises it to %d while running; the manual reproduction command is listed above)" % A2DP_MIN_VOLUME)
    else:
        print("  2) the PC has not established an A2DP music channel yet. Windows only creates an audio output endpoint for the device when A2DP (Audio")
        print("     Sink) is loaded; with only HFP loaded it shows just a microphone:")
        print("       A. Re-pair (most effective): on this machine run")
        print("          sudo python3 %s --forget" % os.path.basename(__file__))
        print("          then click the device on the PC to reconnect")
        print("          or in Windows: Settings -> Bluetooth & other devices -> remove the device -> add it again")
        print("       B. Also confirm these three items on the Windows side (each one can make the output endpoint disappear):")
        print("          * Settings -> Bluetooth & other devices -> More Bluetooth options:")
        print("            check 'Allow Bluetooth devices to play audio' (unchecking it disables all Bluetooth audio output)")
        print("          * Control Panel -> Devices and Printers -> right-click this device -> Properties -> 'Services':")
        print("            check 'Audio Sink' and 'Remote Control',")
        print("            uncheck 'Handsfree' then disconnect and reconnect (this step often forces the output entry out)")
        print("          * services.msc: 'Bluetooth Audio Gateway Service'")
        print("            (BTAGService) must be 'Running'; on Win11 24H2, if")
        print("            'Use LE Audio' is turned on, turn it off")
        print("     after re-pairing, 'Sound settings -> Output' will show two entries for this device:")
        print("       'Headphones (Device name Stereo)'       = A2DP, 44.1/48kHz stereo")
        print("       'Headphones (Device name Hands-Free...)'  = HFP, 16kHz mono")
        print("     if you cannot find it, open the legacy panel: Win+R -> mmsys.cpl -> Playback/Recording")
        print("     (right-click a grayed-out or disabled device to 'Enable' it; you can also 'Show Disconnected Devices' first)")
...
RAW_PEAK_SCRIPT = r"""
# Read a chunk of S16_LE raw recording and print its peak (dBFS) plus whether it
# is digital silence. The leading "level=" tag is parsed by the caller - keep it.
import array
import math
import sys
...
try:
    ...
    print("level=error could not read %s" % path)
    sys.exit(2)
n = len(raw) // 2
samples = array.array("h")
...
for i in range(0, n, 8):
    ...
    if v > peak:
        peak = v
if not peak:
    print("level=silence digital silence (peak 0)")
else:
    print("level=ok peak %.1f dBFS" % (20.0 * math.log10(peak / 32768.0)))
"""


def strip_level_tag(text):
    """去掉采集电平文本里的机器标签，只留给人看的那部分。"""
    if not text:
        return text
    return " ".join(part for part in text.split() if not part.startswith("level="))
...
def verify_mic(dev, channels=CHANNELS):
    ...
    print(f"  Testing recording device {dev} ({channels} channels, {rate} Hz)...")
    cmd = (
    ...
    if result and result.returncode == 0:
        ...
        if level and "level=silence" in level:
            print(f"  !! Microphone {dev} can be opened, but what it captured is "
                  f"{strip_level_tag(level)}:")
            print("     wrong device selected / input muted / the card's capture route has a problem - check amixer's")
            print("     capture-side switches (such as 'Mic2L/Mic2R', 'PGA') and whether the microphone is connected properly")
        print(f"  -> Microphone {dev} is usable ({channels} channels, {rate} Hz"
              f"{(', ' + strip_level_tag(level)) if level else ''})")
        return True
    if result:
        err = (result.stdout or "").strip().splitlines()
        tail = err[-1] if err else "unknown error"
        print(f"  !! {dev} ({channels} channels, {rate} Hz) unavailable: {tail}")
    else:
        print(f"  !! {dev} test timed out")
    return False


def select_and_verify_mic():
    print_status("Probing microphone input devices")
    ...
    if env_dev:
        ...
        print(f"  -> using environment variable MIC_DEVICE={env_dev}")
    ...
    for card, dev, name in usb + other:
        d = f"hw:{card},{dev}"
        print(f"  -> found recording device: {d} ({name})")
...
def main():
    ...
    if not (hasattr(df, "init_df") and hasattr(df, "enhance")):
        log("this df package has no init_df/enhance (PyPI deepfilternet 0.5.x is required), "
            "DeepFilterNet cannot be used, falling back to lightweight spectral subtraction")
        return 2
        ...
        if model_dir in PRETRAINED:
            ...
            if not (os.path.isfile(os.path.join(cache_root, model_dir, "config.ini"))
                    or os.path.isdir(os.path.join(cache_root, model_dir,
                                                  "checkpoints"))):
                log("Model not downloaded: %s/%s is missing config.ini/checkpoints. When online, first"
                    "run it once: sudo %s -c \"from df import init_df; "
                    "init_df('%s')\", or manually unzip the model zip into %s/"
                    % (cache_root, model_dir, sys.executable, model_dir,
                       cache_root))
                return 2
        log("loading DeepFilterNet (model=%s -> %s) ..." % (DF_MODEL, model_dir))
    else:
        log("loading DeepFilterNet (library default model)...")
    try:
        ...
        return 2
    log("model loaded: %s, sr=%d Hz (input %d Hz, built-in polyphase resampling, PF post-filter=%s)"
        % (suffix, df_state.sr(), SR, "on" if POST_FILTER else "off"))
    ...
    if "--benchmark" in sys.argv:
        ...
        for cs in sizes:
            ...
            log("benchmark chunk=%dms rtf=%.2f (threshold %.2f)"
                % (cs, rtf, BENCH_LIMIT))
    ...
    while True:
        ...
        if recover_once and pending_bytes > chunk * step:
            ...
            if dropped:
                log("SCO stall recovered, trimmed %.0f ms of stale audio"
                    % (dropped / step * 1000.0 / SR))
        elif pending_bytes > max_backlog * step:
            dropped = trim_to(max_backlog * step)
            log("backlog over limit, dropped %.0f ms (skip audio to keep latency low)"
                % (dropped / step * 1000.0 / SR))
        ...
        if time.time() - t_start >= 5.0:
            ...
            log("rtf=%.2f backlog=%.0f ms in=%.1f out=%.1f dBFS silence attenuation=%.1f dB%s"
                % (rtf, bl_ms, in_db, out_db, q_db,
                   "" if rtf < 1.0 else " (RTF>1: processing cannot keep up with capture)"))
            ...
            if (
                ...
                log("Sustained RTF=%.2f > 1.15, the model cannot keep up on this machine, "
                    "exiting (10), switching to lightweight noise reduction is recommended" % (cum_proc / cum_audio))
                return 10
            proc_sec = 0.0
            audio_sec = 0.0
            t_start = time.time()
...
def main():
    ...
    pending = []  # 字节块列表，避免大 bytes 反复拼接
    pending_bytes = 0
    log("lightweight spectral denoiser ready, fft=%d hop=%d, backlog limit %d ms"
    ...
    while True:
        ...
        if time.time() - t_start >= 5.0:
            ...
            if quiet_n and quiet_in_pow > 0 and quiet_out_pow > 0:
                q_db = 10.0 * math.log10(quiet_in_pow / quiet_out_pow)
            bl_ms = pending_bytes // step * 1000.0 / SR
            log("rtf=%.2f backlog=%.0f ms in=%.1f out=%.1f dBFS silence attenuation=%.1f dB"
                % (rtf, bl_ms, in_db, out_db, q_db))
            proc_sec = 0.0
            audio_sec = 0.0
            t_start = time.time()
...
def check_pipeline_startup(procs, logs):
    ...
    for label, p in procs:
        if p.poll() is not None:
            reasons.append(f"{label} exited immediately after startup (exit code {p.returncode})")
    if not reasons:
        time.sleep(5)
        for label, p in procs:
            if p.poll() is not None:
                reasons.append(f"{label} exited after startup (exit code {p.returncode})")
    if reasons:
        print_stderr_tails(logs)
        return False, reasons
    print("  -> pipeline health check passed: all child processes are running")
    return True, []
...
def main():
    ...
    while True:
        ...
        if pending_bytes > CAP:
            ...
            print("[gain] backlog exceeded, dropping %.0f ms (SCO stutter recovery)"
...
def apply_capture_pga(mic_device, gain_spec):
    ...
    print(f"  -> analog PGA gain (sound card {card}): {cur if cur else 'unknown'}"
          f"{' (can be raised via MIC_PGA_GAIN, e.g. 20dB)' if not gain_spec else ''}")
    if not gain_spec:
        return cur
...
def ensure_soft_volume_disabled(pcm_path):
    ...
    if res and "boolean true" in (res.stdout or ""):
        print("  !! SoftVolume=true detected, +VGM volume commands will be ignored, turning it off...")
        run(
            f"bluealsa-cli soft-volume {pcm_path} false",
            check=False,
            verbose=False,
        )
        print("  -> SoftVolume is now off; the Windows input volume will genuinely affect the capture gain")
...
def volume_monitor(device, stop_event, lost_event=None):
    ...
    while not stop_event.is_set():
        ...
        if pcm_path:
            ...
            if m:
                ...
                if last_level is None:
                    print(f"  [volume] current SCO microphone gain: {level}/15 ({pct}%)")
                    set_gain_level_file(level)
                elif level != last_level:
                    db = 20 * math.log10(math.sqrt(level / 15.0)) if level > 0 else float("-inf")
                    print(
                        f"  [volume] Windows input volume changed -> "
                        f"Raspberry Pi capture gain {last_level}/15 -> {level}/15"
                        f" ({pct}%, {db:.1f} dB)"
                    )
                    set_gain_level_file(level)
                last_level = level
        ...
        if (
            ...
            print(
                "  [volume] Note: if nothing changes here after you adjust the Windows input volume, "
                "this Windows version does not send the HFP +VGM command"
                " (in that case Windows applies software gain only locally)"
            )
...
def _write_denoise_status(mode, **extra):
    ...
    print("  -> Noise reduction status written to %s" % DENOISE_STATUS_FILE)


def print_denoise_status():
    """--status: 打印当前降噪模式与增益状态 (只读，不启动服务)。"""
    print("=== Noise reduction status ===")
    if os.path.exists(DENOISE_STATUS_FILE):
        ...
        print("  Not recorded yet: the bt-mic service is not running, or the noise reduction mode has not been decided yet")
    if os.path.exists(GAIN_LEVEL_FILE):
        try:
            print("gain_level=%s/15" % Path(GAIN_LEVEL_FILE).read_text().strip())
        except OSError:
            pass
    print("=== Headphone (speaker) status ===")
    if os.path.exists(HEADPHONE_STATUS_FILE):
        try:
        ...
        print("  Not recorded yet: the bt-mic service is not running, or the headphone output has not started")


DF_INFO_CODE = (
    "import df, os, sys; "
    "print('df file:', df.__file__); "
    "print('df version:', getattr(df, '__version__', '?')); "
    "print('full API list:', sorted(a for a in dir(df) if not a.startswith('_'))); "
    ...
    "print('model cache dir:', cache, 'exists' if os.path.isdir(cache) else 'missing'); "
    "found = ([d for d in sorted(os.listdir(cache)) "
    "if os.path.isfile(os.path.join(cache, d, 'config.ini'))] "
    "if os.path.isdir(cache) else []); "
    "print('downloaded models:', found if found else '(none)'); "
...
def print_df_info():
    ...
    print("=== DeepFilterNet installation check ===")
    seen = set()
    for py in denoise_python_candidates():
        ...
        for line in lines:
            print(f"  {line}")
    print("\nNotes:")
    print("  - '0.5.x API (init_df/enhance): True' plus torch available and the model downloaded")
    print("    = the script can enable DeepFilterNet (48kHz model, built-in 16k<->48k resampling)")
    print("  - API present but torch missing or the model not downloaded = install/download it:")
    print("    pip install torch; run init_df once with network access, or place the model zip manually")
    print("  - init_df/enhance and init_model/process both False = no usable df")
    print("    package detected (the script falls back to lightweight spectral subtraction and still works)")
    ...
    with _DF_DECIDE_LOCK:
        ...
        def benchmark_df(sizes):
            ...
            if res is not None:
                ...
                if not rtf_map:
                    print(
                        "  !! Benchmark process exit code %s, df package info (for diagnosis):"
                        % res.returncode
                    )
                    ...
                    if info_lines:
                        for line in info_lines:
                            print(f"    {line}")
                    if res.returncode != 0:
                        print("  !! End of the benchmark process stderr:")
        ...
        if DENOISE_MODE == "df":
            if has_module("df"):
                ...
                print("  !! DENOISE_MODE=df but DeepFilterNet was not detected, degrading")
        ...
        if DENOISE_MODE == "spec":
            if has_module("numpy"):
                ...
                print("  !! numpy is missing, lightweight noise reduction is unavailable, falling back to passthrough forwarding")
                _DF_MODE = None
            _write_denoise_status(_DF_MODE)
            return _DF_MODE
        if DENOISE_MODE == "off":
        ...
        if has_module("df"):
            print(
                "  -> Benchmarking DeepFilterNet processing speed (the first run loads the model, please wait)..."
            )
            sizes = []
            ...
            if rtf_map:
                ...
                if chosen is not None:
                    ...
                    print(
                        "  -> DeepFilterNet passes (chunk=%d ms, rtf=%.2f), enabling DF noise-reduction forwarding"
                    ...
                    return _DF_MODE
                print(
                    "  !! DeepFilterNet exceeds the RTF limit at every chunk size, switching to lightweight spectral subtraction automatically"
                )
            else:
                print("  !! DeepFilterNet benchmark failed, switching to lightweight spectral subtraction automatically")
        elif has_module("numpy"):
            print("  -> DeepFilterNet not detected, using lightweight spectral subtraction")
        else:
            print("  -> No noise-reduction dependencies detected, using passthrough forwarding (no noise reduction)")
            _DF_MODE = None
            _write_denoise_status(None)
            return _DF_MODE
        _DF_MODE = "spec"
...
def _degrade_denoise_mode(venv_python):
    ...
    print("  -> Noise reduction mode degraded to: %s" % (_DF_MODE or "no noise reduction (passthrough forwarding)"))
...
def run_audio_forwarding(bluealsa_pcm, mic_device, mic_channels, device, warmup=None):
    ...
    """
    print_status("Start audio forwarding (keeps running while the Bluetooth connection stays up)")
    ...
    procs = []
    logs = {}
    denoise_notice_done = False   # "降噪可用"提示音每会话只播一次
    ...
    try:
        while True:
            ...
            if not healthy:
                restart_count += 1
                print(f"  !! Startup failed: {'; '.join(reasons)}")
                if restart_count > MAX_RESTARTS:
                    print("  !! Startup failed repeatedly, returning to the waiting state")
                    terminate_all(procs)
                    return "failed"
                terminate_all(procs)
                print(f"  -> Will retry in 3 seconds (attempt {restart_count}/{MAX_RESTARTS})")
                time.sleep(3)
                continue
            restart_count = 0
            ...
            last_bt_check = time.time()
            lost_event.clear()
            if not denoise_notice_done:
                # 管道第一次真正跑起来: 播提示音报"降噪可用" (没启用降噪时
                # mode 为 None，这时候没什么可报的，就不播)
                denoise_notice_done = True
                if mode:
                    play_notice("denoise")
            ...
            while True:
                ...
                if lost_event.is_set() or time.time() - last_bt_check >= 5:
                    last_bt_check = time.time()
                    if not is_device_connected(device):
                        print("  !! Bluetooth disconnected, stopping audio forwarding, returning to the waiting state")
                        log_bt_disconnect_reason()
                        terminate_all(procs)
                        return "disconnected"
                ...
                if not dead:
                    continue
                print("  !! A forwarding child process exited")
                for label, p in dead:
                    print(f"    -> {label} exit code: {p.returncode}")
                print_stderr_tails(logs)
                ...
                if mode == "df" and any(
                    ...
                    print("  -> DeepFilterNet cannot keep up in real time, switching to lightweight spectral subtraction automatically")
                    terminate_all(procs)
                    _degrade_denoise_mode(venv_python)
                    mode = _DF_MODE
                    pipeline = make_pipeline()
                    print("  New pipeline commands:")
                    for label, cmd in pipeline:
                        print(f"    {label}: {' '.join(cmd)}")
                    restart_count = 0
                    time.sleep(1)
                ...
                restart_count += 1
                if restart_count > MAX_RESTARTS:
                    print("  !! Maximum restart count exceeded, returning to the waiting state")
                    terminate_all(procs)
                    return "failed"
                terminate_all(procs)
                print(
                    f"  -> Restarting the forwarding pipeline in 2 seconds (attempt {restart_count}/{MAX_RESTARTS})"
                )
                time.sleep(2)
                break
...
def install_systemd_unit():
    print_status("Installing the systemd auto-start service bt-mic.service")
    ...
    with open(SERVICE_UNIT_PATH, "w", encoding="utf-8") as f:
        f.write(unit)
    print(f"  -> Wrote {SERVICE_UNIT_PATH}")
    ...
    if status and status.stdout.strip() == "active":
        print("  -> bt-mic.service installed and (re)started (active); it will run automatically on boot")
        print("     the service was restarted this time, so the new script takes effect immediately")
        print("  View logs: journalctl -u bt-mic -f")
    else:
        print("  !! The service may not have started successfully, please check:")
        print("     systemctl status bt-mic.service")
        print("     journalctl -u bt-mic -n 100")


def uninstall_systemd_unit():
    print_status("Uninstalling the systemd service bt-mic.service")
    ...
    run("systemctl daemon-reload", check=False, verbose=False)
    print("  -> Uninstalled (the script file itself is kept; you can --install again at any time)")
    ...
    print_status("Restoring the Bluetooth override config and disconnecting")
    disconnect_bluetooth_devices()
    _restore_bluetooth_overrides()
    ...
    print("  -> Bluetooth disconnected and powered off (pairing is kept; the next time this program runs, Windows can reconnect directly)")
...
def main():
    ...
    print("=== Raspberry Pi Bluetooth microphone (BlueALSA + DeepFilterNet), complete edition ===")
    print("Waits for Windows to connect; PIN-less pairing; keeps running after a disconnect, waiting to be reconnected")
    print("Also works as a Bluetooth headset/speaker: PC audio is routed to the headphone jack (chosen automatically, preferring a card different from the microphone's")
    print(" sound card; if you switched headphone jacks and hear nothing, run --find-output to re-confirm which jack it is)")
    print("Build version: %s" % BUILD_ID)
    if _CONFIG_APPLIED:
        print("Loaded config %s: %s"
              % (CONFIG_FILE, ", ".join("%s=%s" % kv for kv in _CONFIG_APPLIED)))
    else:
        print("(no config file %s; to change settings just write KEY=VALUE into it, or run --find-output)"
              % CONFIG_FILE)
    check_script_updated()
    ...
    try:
        ...
        if os.path.exists("/etc/asound.conf"):
            ...
            print("  -> Moved /etc/asound.conf out of the way")
        ...
        while True:
            ...
            if mic_result[0]:
                ...
                break
            print("  !! No recording device found, retrying in 5 seconds (the service keeps running and Bluetooth stays discoverable)")
            ensure_discoverable()
            time.sleep(5)
        print(f"  -> Final microphone: {mic_device} ({mic_channels} channels)")
        if mic_device:
        ...
        cross_route = False
        if HEADPHONE_ENABLE:
            print_status("Probing headphone (speaker) output devices")
            playback_device, play_why = select_playback_device(mic_device)
            if playback_device:
                print(f"  -> Headphone jack playback device: {playback_device} ({play_why})")
                ...
                print(f"  !! No usable playback device found ({play_why}): ")
                print("     Bluetooth headphone (speaker) output is unavailable; the microphone is not affected")
        else:
            print("  -> HEADPHONE_ENABLE=0: Bluetooth headphone (speaker) output disabled")
        ...
        while True:
            ...
            if get_connected_devices():
                ...
                print_status(f"Session {session}: device is still connected, trying to resume forwarding directly")
            else:
                print_status(f"Session {session}: waiting for Windows to connect")
                ensure_discoverable()
            ensure_pairing_agent()
            ...
            trust_device(device)
            # 蓝牙已连上:播"已连接"提示音。放在起耳机输出线程之前，这样此时
            # 声卡还没被下行管道占用，提示音一定能出声
            play_notice("bt")
            ...
            hp_thread.start()
            try:
                # PCM 等待期间预启动降噪进程:模型加载 (5~7 seconds)与等待并行，
                ...
                while is_device_connected(device):
                    ...
                    if pcm:
                        break
                    print("  -> The connection is still up but there is no SCO PCM yet, retrying later...")
                    time.sleep(2)
                if not pcm:
                    print("  -> Bluetooth disconnected while waiting for the PCM, returning to the waiting state")
                    stop_denoise_warmup(warmup)
                    continue

                print(f"  -> Final PCM: {pcm}")
                reason = run_audio_forwarding(
                    pcm, mic_device, mic_channels, device, warmup=warmup
                )
                print_status(f"Session {session} ended (reason: {reason})")
                if reason == "interrupted":
                    raise KeyboardInterrupt
                print("  -> The program keeps running and Bluetooth is discoverable again, waiting for the next connection")
                time.sleep(3)
            finally:
                hp_stop.set()
                hp_thread.join(timeout=8)
                print("  -> Headphone output stopped (this session has ended)")
...
def disable_wifi_powersave():
    ...
    if WIFI_POWERSAVE != "off":
        print("  -> WIFI_POWERSAVE=%s: keeping the system WiFi power-save setting" % WIFI_POWERSAVE)
        return
    if shutil.which("iw") is None:
        print("  -> iw is not installed, skipping the WiFi power-save change (you can apt install iw)")
        return
    ...
    for iface in ifaces:
        ...
        if r and r.returncode == 0:
            print(f"  -> Turned off WiFi power save on {iface} (reduces Bluetooth audio stutter/dropouts)")
        else:
            print(f"  !! Failed to turn off WiFi power save on {iface} (this does not affect other features)")
...
def set_performance_governor():
    ...
    for path in governors:
        try:
            ...
            pass
    if changed:
        print("  -> CPU governor set to performance (keeps real-time noise reduction speed steadier)")
