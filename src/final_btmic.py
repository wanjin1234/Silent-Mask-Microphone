...
PLAYBACK_DEVICE_FORCE = os.environ.get(
    "PLAYBACK_DEVICE_FORCE", "0"
).strip().lower() not in (
    "0",
    "false",
    "off",
...
HEADPHONE_CROSS_ROUTE = os.environ.get(
    "HEADPHONE_CROSS_ROUTE", "1"
).strip().lower() not in (
    "0",
    "false",
    "off",
...
# 东西，不能因为它把服务搞崩），0 = 静音
NOTIFY_TONE_VOLUME = min(
    1.0, max(0.0, float(os.environ.get("NOTIFY_TONE_VOLUME", "0.3")))
)
...
HEADPHONE_STATUS_FILE = (
    "/tmp/bt_headphone_status"  # 耳机 (扬声器)输出状态 (--status 可查)
)
...
def check_script_updated():
    ...
    if mtime > _SCRIPT_MTIME[0] + 1 and not _UPDATE_WARNED[0]:
        ...
        _SCRIPT_MTIME[0] = mtime
        print(
            "  !! The script on disk was updated, but the current process is still running the old code:"
        )
        print(
            "     Please run sudo systemctl restart bt-mic (restart this program if it runs in the foreground)"
        )
...
def run(cmd, check=False, timeout=60, verbose=True):
    ...
    try:
        ...
        if check and result.returncode != 0:
            ...
            print(result.stderr)
            raise RuntimeError(
                f"Command failed (returncode={result.returncode}): {cmd}"
            )
        return result
...
def remove_paired_devices():
    ...
    if not result or result.returncode != 0:
        print(
            "  !! Cannot read the pairing list (adapter not ready?); the link will drop after a system reboot"
        )
        return
    for line in result.stdout.splitlines():
        parts = line.split()
        if len(parts) >= 2 and parts[0] == "Device":
            ...
            print(
                f"  -> Removed paired device {mac} (Windows auto-reconnect will fail)"
            )
...
def forget_paired_devices():
    ...
    """
    print(
        "=== Removing the pairing on the Raspberry Pi side to force the PC to re-pair and rediscover services ==="
    )
    print(
        "  The PC side needs no device removal: it will re-pair (PIN-less) and re-enumerate services while pairing"
    )
    remove_paired_devices()
    set_discoverable(True)
    print("  -> Now click that device in the PC's Bluetooth settings to reconnect")
    print(
        "  -> After re-pairing, 'Sound settings -> Output' will show 'Headphones (device name Stereo)',"
    )
    print(
        "     that is the A2DP music route (which sends PC audio to the 3.5mm headphone jack)"
    )
...
def setup_packages():
    ...
    if not missing:
        print_status(
            "Dependency check passed (bluez/bluealsa already installed, skipping apt)"
        )
        if shutil.which("bluealsa-cli") is None:
            print(
                "  !! bluealsa-cli is missing: diagnostics and volume control are unavailable"
            )
            print(
                "     Please run sudo apt install bluez-alsa-utils (the microphone is unaffected)"
            )
        return
    print_status(
        f"Checking and installing dependencies (missing: {', '.join(missing)})"
    )
...
def ensure_bluealsa_dbus(hci):
    ...
    if not ba_path:
        return False
    print(
        "  !! Still unavailable, continuing with a direct start of the bluealsa daemon with explicit arguments"
    )
...
def resolve_uuid_conflict(hci):
    print_status(
        "Resolving the UUID conflict: stop the conflicting services and restart bluetooth/bluealsa"
    )
    run("systemctl stop bluealsa 2>/dev/null || true", check=False, verbose=False)
    ...
    if not (check and "--noplugin=audio,headset" in (check.stdout or "")):
        print(
            "  !! Note: bluetoothd is not running with --noplugin=audio,headset, it may be unable to release the UUID"
        )
    ...
    print(
        "  !! The UUID conflict still exists; the processes/services that may occupy the UUID are:"
    )
...
def configure_main_conf():
    ...
    print_status(
        "Configuring /etc/bluetooth/main.conf (device class/discoverability/PIN-less re-pairing)"
    )
    if not os.path.exists(BLUETOOTH_CONF):
        print(
            "  !! main.conf not found, skipping (runtime bluetoothctl settings still take effect)"
        )
        return
...
def configure_bluetoothd():
    ...
    if check and "--noplugin=audio,headset" in (check.stdout or ""):
        ...
        print(
            "  !! Warning: --noplugin=audio,headset not detected, a plugin conflict may still exist"
        )
...
def log_bt_disconnect_reason(device=None):
    ...
    if len(hits) <= 10 and len(lines) > 10:
        print(
            "    （以上是关键字匹配；下面是 BlueZ 原始日志末尾，供无 Reason 行时排查）"
        )
        for line in lines[-8:]:
            print(f"    {line}")
    if device:
        ...
        if rssi is not None:
            ...
            try:
                if int(rssi) < -75:
                    ...
                    print(
                        "     共用一颗芯片和天线，2.4GHz WiFi 忙时正是这种表现"
...
def configure_bluealsa():
    ...
    if not ba_path:
        raise FileNotFoundError(
            "bluealsa not found, please make sure bluez-alsa is installed"
        )
    ...
    if BLUEALSA_EXTRA_ARGS:
        ...
        print(
            "  -> Appending bluealsa arguments (BLUEALSA_EXTRA_ARGS): %s"
            % BLUEALSA_EXTRA_ARGS
        )
    a2dp_candidates = bluealsa_a2dp_profile_name(ba_path) if A2DP_SINK_ENABLE else []
    if not A2DP_SINK_ENABLE:
        print(
            "  -> A2DP_SINK_ENABLE=0: only HFP/HSP is registered (the PC can only play audio through the Hands-Free route)"
        )
    ...
    for name in a2dp_candidates:
        ...
        if not restart_bluealsa():
            print(
                f"  !! Starting bluealsa with A2DP profile '{name}' failed, trying other names"
            )
        ...
        if a2dp_sdp_ok is False:
            print(
                f"  !! profile '{name}' started successfully, but there is no Audio Sink record in the SDP,"
            )
    ...
    if a2dp_profile is None:
        ...
        if a2dp_candidates:
            print(
                "  !! Failed to register A2DP Sink, fell back to HFP/HSP only (microphone first)"
            )
    _A2DP_REGISTERED[0] = a2dp_profile
    print(f"  -> Using arguments: {new_exec}")
    if a2dp_profile:
        print(
            f"  -> A2DP Sink registered ({a2dp_profile}): the PC can use the Raspberry Pi as headphones/speaker"
        )
    ...
    if result and result.stdout.strip() == "active":
        ...
        print(
            "  !! BlueALSA service is not active, trying to start the Bluetooth HF/HS mode directly"
        )
        run(
    ...
    if "UUID already registered" in journal_text:
        ...
        if "Couldn't register hands-free profile" in journal_text:
            ...
            print(
                "  !! A2DP UUID conflict detected (the headphone feature needs it), trying to clean up..."
            )
            resolve_uuid_conflict(hci)
        else:
            # 只有 A2DP UUID 冲突，不影响 HFP/HSP 麦克风功能
            print(
                "  -> A2DP UUID conflict only, it does not affect the HFP/HSP microphone route, you can continue"
            )
    else:
    ...
    if sdp_ok is False:
        print(
            "  !! The local SDP is missing the Handsfree record, restarting bluealsa to re-register..."
        )
        run("systemctl restart bluealsa", check=False, verbose=False)
        time.sleep(3)
        sdp_ok = verify_hfp_sdp_record()
    if sdp_ok is True:
        print(
            "  -> Confirmed the local SDP contains the Handsfree service record (Windows can discover HFP)"
        )
    elif sdp_ok is False:
        print(
            "  !! The Handsfree SDP record is still missing after the restart: Windows will not see HFP when pairing,"
        )
        print(
            "     and the Raspberry Pi cannot be selected as a microphone. Please check journalctl -u bluealsa."
        )
    ...
    if a2dp_profile:
        if a2dp_sdp_ok is True:
            print(
                "  -> Confirmed the local SDP contains the Audio Sink record (the PC can use the Raspberry Pi as headphones/speaker)"
            )
        else:
            ...
            if a2dp_ok is False:
                print(
                    "  !! The local SDP is missing the Audio Sink (A2DP) record, restarting bluealsa to re-register..."
                )
                run("systemctl restart bluealsa", check=False, verbose=False)
                time.sleep(3)
                a2dp_ok = verify_a2dp_sdp_record()
            if a2dp_ok is True:
                ...
                print(
                    "  !! The Audio Sink SDP record is still missing: Windows sound settings will not show a stereo output entry,"
                )
                print(
                    "     the headphone jack can only use the Hands-Free downlink. Please check journalctl -u bluealsa."
                )
...
def start_pairing_agent():
    ...
    if _AGENT_PROC is not None:
        ...
        print(
            "  -> Pairing agent running persistently (NoInputNoOutput), PIN-less pairing with Windows is ready"
        )
    else:
        print(
            "  !! Failed to start the pairing agent, it will be retried automatically later"
        )
...
def recover_bt_adapter():
    ...
    print(
        "  !! Bluetooth controller missing, trying to recover (restart hciuart / bring up hci0 / power on)..."
    )
    run(
...
def wait_for_bt_adapter(timeout=90):
    print_status(
        "Waiting for the Bluetooth adapter to be ready (this can take a few seconds at boot)"
    )
    start = time.time()
    while time.time() - start < timeout:
        ...
        time.sleep(3)
    print(
        f"  !! Bluetooth adapter not ready after {timeout} seconds, continuing to run (the background keeps retrying)"
    )
    return False
...
def set_discoverable(on):
    if on:
        ...
        for attempt in range(3):
            ...
            if state == (True, True):
                print(
                    "  -> Bluetooth is now discoverable/pairable, waiting for Windows to connect"
                )
                return True
            time.sleep(1)
            run("bluetoothctl discoverable on", check=False, verbose=False)
        ...
        if state is None:
            print(
                "  !! Bluetooth controller does not exist (adapter not ready?), it will be retried automatically later"
            )
            recover_bt_adapter()
        else:
            print(
...
def wait_for_connection(timeout=CONNECTION_TIMEOUT):
    if timeout > 0:
        ...
        print_status(
            "Waiting for Windows to connect (wait indefinitely, Ctrl+C to stop)"
        )
    print(
        "  PIN-less pairing: add a device in Windows Bluetooth settings and select the Raspberry Pi, no pairing code required"
    )
    print(
        "  If Windows still shows a PIN prompt, first 'Remove device' in Windows Bluetooth settings and add it again"
    )
    print(
        "  After connecting, go to Windows 'Sound settings -> Input' and select the Hands-Free/Headset device,"
    )
    print("  only then will BlueALSA establish the HFP/SCO microphone route.")
    print(
        "  To play sound through the reSpeaker 3.5mm headphone jack: go to Windows 'Sound settings -> Output'"
    )
    print(
        "  and select this device ('... Stereo' uses A2DP stereo, '... Hands-Free' uses the 16kHz route),"
    )
    print(
        "  both produce sound; if there is no stereo entry in sound settings, the device must be removed and re-paired."
    )
    start = time.time()
    last_notice = 0
    last_ensure = 0
    while True:
        ...
        if time.time() - last_notice >= 30:
            print(
                "  Still waiting for Windows to connect... (the program keeps running)"
            )
...
def dump_pcm_diagnostics():
    ...
    for cmd in (
        ...
        res = run(cmd, check=False, timeout=8, verbose=False)
        print(
            f"  $ {cmd}  ->  rc={res.returncode if res is not None else '(timed out)'}"
        )
        if res:
...
def find_working_pcm(device, timeout=90):
    print_status(
        f"Waiting for the BlueALSA SCO PCM to appear (this round, up to {timeout} seconds)"
    )
    ...
    while time.time() - start < timeout:
        ...
        if detailed:
            ...
            if slc_up:
                print(
                    "    SLC is up but the PCM still cannot be opened: the channel may be held exclusively by a leftover aplay,"
                )
                print(
                    "    or Windows has not enabled recording yet (the SCO is only created after you select it in Sound settings -> Input)."
                )
            else:
                print(
                    "    Windows has not initiated the HFP service-level connection (SLC) yet. It only initiates it for the"
                )
                print(
                    "    services it cached at pairing time: if HFP is not in that cache, it will never initiate it (see the guidance below)."
                )
        ...
        if round_no % 5 == 0:
            ...
            if kill_bt_profile_conflicts():
                if _audio_stream_busy(device):
                    print(
                        "    -> 清理了重新出现的冲突进程，但电脑正在用 A2DP 播放"
                        "音频：不重启 bluealsa（重启会把正在放的音乐掐断）"
                    )
                else:
                    print(
                        "    -> 清理了重新出现的冲突进程，重启 bluealsa 重新注册 profile"
                    )
                    run("systemctl restart bluealsa", check=False, verbose=False)
                    time.sleep(3)
        ...
        if (
            ...
            if _audio_stream_busy(device):
                ...
                if not allowed:
                    ...
                    print(
                        "  -> 仍无 HFP 服务级连接：主动断开蓝牙**一次**，触发 Windows"
                    )
                    ...
                    print(
                        "     若重连后依旧如此，说明 Windows 缓存的设备服务里没有 HFP："
                    )
                    print(
                        "     请在 Windows 蓝牙设置中删除该设备，等本程序显示可发现后"
                    )
    ...
    # 额外诊断
    print(
        "  !! no usable PCM found in this round, requesting the HFP profile on the link again"
    )
    request_hfp_profile(device)
    time.sleep(5)
    ...
    print(
        "     2. after pairing, open 'Sound settings' and select Hands-Free/Headset as the input device;"
    )
    print(
        "     3. open any recording app so that Windows really enables the microphone channel;"
    )
    print(
        "     4. check whether Windows sees the Pi as 'Headphones' instead of 'Speakers'."
    )
    if not diag_done:
        dump_pcm_diagnostics()
    return None
...
def select_playback_device(mic_device=None):
    ...
    if PLAYBACK_DEVICE:
        ...
        if ok and same_as_mic and not PLAYBACK_DEVICE_FORCE:
            ...
            print(
                "  !! 配置的 PLAYBACK_DEVICE=%s 与麦克风（%s）是**同一张声卡**："
                % (PLAYBACK_DEVICE, mic_device)
            )
            print(
                "     ① 声音只会从被选中那张卡的 3.5mm 孔出来——耳机插在另一个"
                "孔上就完全没声音；"
            )
            print(
                "     ② 同一张卡的放音与采集只能同采样率，同时用会互相挤"
                "（实测：放音持续 underrun、麦克风 overrun，两边都卡）"
            )
            print(
                "     已改为自动重挑一张与麦克风不同卡的声卡（原因见下）；"
            ...
            print(
                "     改配置：sudo nano %s 或 sudo python3 %s --find-output"
    ...
    if PLAYBACK_PREFER in groups:
        print(
            "  -> PLAYBACK_PREFER=%s：这一类声卡优先（含与麦克风同卡的）"
            % PLAYBACK_PREFER
        )
        ordered = list(groups[PLAYBACK_PREFER])
        rest = [g for g in ("hat", "other", "onboard", "hdmi") if g != PLAYBACK_PREFER]
    else:
        if PLAYBACK_PREFER not in ("", "any"):
            print(
                "  !! PLAYBACK_PREFER=%s 不是有效取值（hat/onboard/hdmi/other/any），"
    ...
    for card, dev, label, same in ordered:
        ...
        print(
            "  -> playback device: %s (%s) %s%s"
            % (
                cand,
                label,
                "usable" if ok else "not usable: " + err,
        ...
        if same:
            print(
                "  !! this sound card is also the one the microphone is on: both directions share one I2S clock on the codec,"
            )
            print(
                "     so they can only use the same sample rate (the microphone is fixed at %d Hz, and A2DP's 44.1/48kHz fights with it)"
                % SAMPLE_RATE
            )
            print("     workarounds (pick one of the two, try each once):")
            print(
                "       HEADPHONE_CAPTURE_RATE=44100   capture the microphone at 44.1kHz (playback is preserved)"
            )
            print(
                "       HEADPHONE_HAT_MODE=1           drop the downlink to 16kHz mono"
            )
            print(
                "     the zero-trade-off option: plug the headphones into another sound card (the Pi's onboard 3.5mm / USB),"
            )
            print(
                "     or run sudo python3 %s --find-output and let it confirm which card the headphones are plugged into"
                % os.path.basename(__file__)
            )
        else:
            print(
                "  -> not the same sound card as the microphone: the two directions do not affect each other (recommended)"
            )
        return cand, label
    # 一张都打不开:给出板载音频没启用的可能
    names = [l for _c, _d, l in probe_playback_devices()]
    print(
        "  playback devices in aplay -l: %s"
        % (", ".join(names) if names else "(none at all)")
    )
    if not names:
        print(
            "  !! no playback device at all: onboard audio may be turned off. To get sound out of the Pi's onboard 3.5mm"
        )
        print(
            "     jack, make sure /boot/firmware/config.txt contains dtparam=audio=on"
        )
        print(
            "     (or /boot/config.txt depending on the OS version), then reboot and try again"
        )
...
def read_playback_mixer(playback_device):
    ...
    if not found:
        ...
        print(
            f"  mixer controls available on sound card {card}: {', '.join(names) if names else '(cannot read)'}"
        )
...
def dump_all_mixer_controls(playback_device, limit=40):
    ...
    names = list_mixer_control_names(card)
    print(
        "-- all mixer controls of sound card %s (read-only; %d in total) --"
        % (card, len(names))
    )
    if not names:
    ...
    for ctl in names[:limit]:
        ...
        values = [
            ...
            if ln.strip()
            and not ln.strip().startswith(("Simple mixer control", "numid="))
...
def _mixer_state_token(raw):
    ...
    for part in mixer_control_channels(raw):
        if "%" in part:
            ...
            continue
        states = [
            t for t in re.split(r"[,\s]+", part.strip().lower()) if t in ("on", "off")
        ]
        if states:
...
def apply_playback_mixer(playback_device, cross_route=False, quiet=False):
    ...
    if not done and not failed and not quiet:
        ...
        print(
            "     if the headphones have no sound, use alsamixer -c %s to adjust the volume/unmute manually"
            % card
        )
...
def decide_hat_mode(playback_device, mic_device, mic_channels=CHANNELS):
    ...
    if not same_sound_card(playback_device, mic_device):
        print(
            "  -> the microphone (%s) and the headphone jack (%s) are not on the same sound card: the two directions do not share a clock,"
            " and do not affect each other" % (mic_device, playback_device)
        )
        return False
    print(
        "  !! 麦克风与耳机口是同一张声卡（%s）：收发共用 codec 的一路 I2S 时钟，"
        "两个方向只能同一个采样率" % playback_device
    )
    print(
        "     麦克风走 HFP 固定 %d Hz，A2DP 音乐是 44100/48000 Hz —— 同时使用时"
        "会互相挤（实测：放音持续 underrun、麦克风 overrun，两边都卡）" % SAMPLE_RATE
    )
    if MIC_CAPTURE_RATE != SAMPLE_RATE:
        print(
            "  -> 已设 HEADPHONE_CAPTURE_RATE=%d：麦克风按该采样率采集，上行链路"
    ...
    if CAPTURE_RATE_EXPLICIT:
        print(
            "  -> 已显式指定 HEADPHONE_CAPTURE_RATE=%d（与 SCO 同率）：按你的设置"
            "采集，不做自动调整" % MIC_CAPTURE_RATE
        )
        print(
            "     同卡时两个方向只能有一个采样率：若放音被采集挤掉，删掉这行配置"
    ...
    if opt in ("1", "on", "true", "yes"):
        ...
        for rate, ch, label in ((SAMPLE_RATE, 1, "mono"), (SAMPLE_RATE, 2, "stereo")):
            ...
            tested.append(
                "%d Hz %s %s" % (rate, label, "usable" if ok else "not usable")
            )
            if ok:
                _HAT_OUT[:] = [rate, ch]
                print(
                    "  -> same-card compatibility mode (HEADPHONE_HAT_MODE=%s): headphone downlink unified to "
                    "%d Hz %s" % (opt, rate, label)
                )
                print(
                    "     note: on this card %d Hz playback may be silent in practice (it opens successfully but cannot be heard),"
                ...
                return True
        print(
            "  !! the headphone jack does not support %d Hz playback, so the rate cannot be unified with the capture rate; self-test: %s"
    ...
    if matched:
        ...
        print(
            "  -> 已自动同卡共存：麦克风改按 %d Hz 采集（上行链路内置重采样到 "
            "%d Hz 送 SCO），放音保持原生采样率——两个方向同率，不再抢时钟"
            % (matched, SAMPLE_RATE)
        )
        print(
            "     （等价于手工配置 HEADPHONE_CAPTURE_RATE=%d；想改回 16kHz 采集就"
            "显式写它）" % matched
        )
        return False
    print(
        "  !! 麦克风无法按 44100/48000 Hz 采集，只剩 %d Hz 采集可用："
        "放音与采集只能有一个采样率" % SAMPLE_RATE
    )
    print("     两个对策（各试一次，用 --hp-test 听有没有声音）：")
    print(
        "       HEADPHONE_HAT_MODE=1             下行降到 16kHz 单声道（本卡可能无声）"
    )
...
def bluealsa_pcm_format(pcm_path, profile):
    ...
    """
    res = run(f"bluealsa-cli info {pcm_path}", check=False, timeout=8, verbose=False)
...
def build_downlink_player(
    device, playback_device, python=None, prefer=None, hat_mode=False, shared_rate=None
):
    ...
    if hat_mode:
        ...
        print(
            "  -> 同卡共存：下行 %d Hz %d ch → 重采样到采集采样率 %d Hz 单声道"
    ...
    if need_mix:
        try:
            ...
            print(
                "  !! cannot write the downlink mix script (%s): skipping sample rate unification this time"
                % exc
            )
            out_rate, out_channels = rate, channels
...
def print_a2dp_silent_hint():
    ...
    print(
        "  !! the downlink level is almost digital silence: the audio was scaled to ~0 before it reached the Pi,"
    )
    print(
        "     the problem is not the headphone jack/mixer (the local test tone is fine), check in this order:"
    )
    print("     A. PC side (most common):")
    print(
        "        * Sound settings -> Output -> open this device and check whether its volume slider is pulled down very low;"
    )
    print(
        "        * Control Panel -> Sound -> 'Communications' tab: by default, when it detects communication activity it"
    )
    print(
        "          'reduces the volume of other sounds by 80%' or even mutes them - and this machine is also a microphone"
    )
    print(
        "          (a communications device), so as soon as the microphone is on the PC ducks the music. Change it to"
    )
    print("          'Do nothing'.")
    print(
        "     B. Raspberry Pi side (A2DP volume model): currently SoftVolume=false and the bluealsa"
    )
    print(
        "        startup arguments have no --a2dp-volume - an undefined half-state, in which samples may be scaled by the AVRCP"
    )
    ...
    print(
        "        * Let the PC slider control the volume: add a drop-in to the service"
    )
    print("          Environment=BLUEALSA_EXTRA_ARGS=--a2dp-volume")
    print(
        "        (for the A2DP PCM path, see the output of sudo python3 %s --audio-info)"
        % os.path.basename(__file__)
    )
...
def force_local_a2dp_volume(device):
    ...
    if not path:
        return False
    print(
        "  -> Switching the A2DP downlink to local volume control: soft-volume true + volume %d"
        % A2DP_MIN_VOLUME
    )
    ...
    vol = read_bluealsa_volume(path)
    print(
        "     Now: SoftVolume=%s volume=%s (if the headphones are still silent, watch the level meter: if the level does not change,"
...
def headphone_monitor(
    ...
    for tool in ("arecord", "aplay"):
        if shutil.which(tool) is None:
            print(
                "  !! Missing %s (apt install alsa-utils); Bluetooth headphone output unavailable"
                % tool
            )
    ...
    if playback_device is None:
        ...
        if playback_device is None:
            print(
                f"  !! No headphone jack playback device found ({why}): Bluetooth headphone function skipped"
            )
    ...
    print(
        "  -> Bluetooth headphone output is ready (select this machine as the output device on the PC and you will hear sound)"
    )
    print(
        "     Playback method: arecord <downlink PCM> | aplay -D %s%s"
        % (
            playback_device,
    ...
    write_headphone_status(
        ...
        player="hp-rec|hp-mix|hp-meter|hp-play"
        if hat_mode
        else "hp-rec|hp-meter|hp-play",
    ...
    def wait_link_back(where):
        ...
        """
        print(
            "  -> 耳机输出：蓝牙连接已断开（%s），停止播放并等待链路恢复，最多 %d 秒"
        ...
        if stop_event.is_set():
            ...
            print(
                "  -> 耳机输出：链路 %d 秒未恢复，本会话不再尝试（下次连接重新开始）"
    ...
    def switch_downlink_route(reason, target=None, force=False):
        ...
        if prof is None:
            print(
                "  -> The other downlink route does not exist right now, cannot switch yet (waiting for the PC to establish it)"
            )
            return False
        if pcm is None:
        ...
        if not force:
            if not pcm:
                print(
                    "  -> Cannot get the PCM name of the other downlink route (the list tool did not report it), not switching for now"
                )
                return False
            now = time.time()
            ...
            if note != probe_note:
                ...
                if live:
                    print(
                        "  -> Liveness probe result: audio data was read on %s"
                        % describe_route(prof)
                    )
                else:
                    print(
                        "  -> Liveness probe result: no audio data can be read on %s either (the PC is not sending audio to that one),"
        ...
        if route_switches >= MAX_ROUTE_SWITCHES:
            if not cap_noted:
                cap_noted = True
                print(
                    "  -> Switched %d times already, no more automatic switching so it does not hop back and forth"
                    " (you can pin the route manually with HEADPHONE_PROFILE)"
                    % route_switches
                )
            return False
        print(
            "  -> Downlink route switch (%s): %s -> %s"
        ...
        while not stop_event.is_set():
            ...
            if time.time() - last_a2dp_check >= A2DP_CHECK_SECS:
                ...
                if a2dp_state is None:
                    ...
                    if active:
                        print(
                            "  -> A2DP music channel detected (the PC is streaming audio over)"
                        )
                ...
                if not active and not no_a2dp_hint and _A2DP_REGISTERED[0]:
                    no_a2dp_hint = True
                    print(
                        "  !! The PC is connected, but the A2DP music channel has not been established yet:"
                    )
                    print(
                        "     In Windows, only when it has cached this machine's Audio Sink service will"
                    )
                    print(
                        "     'Sound settings -> Output' show 'Headphones (device name Stereo)'."
                    )
                    print(
                        "     If the output list only shows a microphone / no such device, the service list used at pairing time"
                    )
                    print(
                        "     is stale: delete the device in Windows and pair again, or run on the Raspberry Pi"
                    )
                    print(
                        "     sudo python3 %s --forget so that the PC pairs again."
                        % os.path.basename(__file__)
                    )
                    print(
                        "     For troubleshooting: sudo python3 %s --audio-info"
                        % os.path.basename(__file__)
                    )
            ...
            if active_now != uplink_seen:
                ...
                if active_now:
                    print(
                        "  -> Microphone uplink started: rechecking whether the headphone jack mixer was"
                        " reset to default by the sound card driver"
                    )
                last_mixer_check = 0.0  # 立刻检查一次
            if (
                playback_device
                and _HP_MIXER_APPLIED
                ...
                if drift:
                    print(
                        "  !! Headphone jack mixer was reset to default by the sound card driver (this happens when the capture stream is opened / the "
                    ...
                    if mixer_refix < 5:
                        ...
                        print(
                            "  -> Headphone jack mixer re-applied (attempt %d)"
                            % mixer_refix
                        )
                    elif not mixer_giveup_noted:
                        mixer_giveup_noted = True
                        print(
                            "  !! The mixer keeps being reset (fighting back and forth with the driver), no longer re-applying it automatically."
                        )
                        print(
                            "     You can verify manually with alsamixer -c %s, or plug the headphones into"
            ...
            if procs and time.time() - last_downlink_check >= DOWNLINK_CHECK_SECS:
                ...
                if not progressed and uptime >= DOWNLINK_STALL_SECS:
                    ...
                    if not stall_noted:
                        ...
                        print(
                            "  !! 耳机输出：%s 上没有新数据了（已运行 %.0f 秒，"
                            "累计字节 %s）"
                            % (
                                describe_route(cur_profile),
                                uptime,
                    ...
                    print(
                        ...
                        % (
                            "fully silent" if level == "silence" else "too low",
                            silent_strikes,
                        )
                    )
                    if silent_strikes == 2 and not volume_fix_done:
                        ...
                        print(
                            "     If the level changes along with it -> it is a volume model problem on the Raspberry Pi side;"
                        )
                        print(
                            "     If the level does not change at all -> the amplitude is being suppressed on the PC side (see the guidance below)"
                        )
                    ...
                    if silent_strikes:
                        print(
                            "  -> Downlink level is back to normal (the peak is now visible), no longer counted as silent"
                        )
                    silent_strikes = 0
                    silent_hint_shown = False
            ...
            if not procs:
                ...
                if player is None:
                    if not no_pcm_noted:
                        no_pcm_noted = True
                        print(
                            "  -> Headphone output: no usable downlink PCM on this machine right now (the PC has not"
                        )
                        print(
                            "     selected this machine as the output device, or has not set up an A2DP/Hands-Free route yet)"
                        )
                    restart = 0
                    stop_event.wait(3)
                    continue
            ...
            if restart >= 2 and switch_downlink_route(
                "the player keeps failing to start", force=True
            ):
                restart = 0
                continue
            if restart > MAX_RESTARTS:
                ...
                print(
                    "  !! Headphone output failed to start %d times in a row: retrying in 30 seconds"
                    " (not giving up on this session, microphone function is unaffected)"
                    % restart
                )
                write_headphone_status(
                    state="retrying", playback_device=playback_device
                )
                restart = 0
                stop_event.wait(30)
                continue
...
def ensure_a2dp_audible(device):
    ...
    if vol is None:
        print(
            "  -> The A2DP downlink volume attribute is unreadable (SoftVolume=%s)"
            % _soft_volume_text(soft)
        )
        print(
            "     Not touching the stream that is playing (in practice, changing the soft volume makes the stream go silent);"
        )
        print(
            "     If there really is no sound, you can try the commands listed in --audio-info by hand"
        )
        return note
    if vol > 0:
        print(
            "  -> A2DP downlink volume=%s SoftVolume=%s (normal)"
            % (vol_text, _soft_volume_text(soft))
        )
        return note
    print(
        "  !! A2DP downlink volume=0: the whole stream will be scaled to silence, raising it to %d"
        % A2DP_MIN_VOLUME
    )
    res = run(
        f"bluealsa-cli volume {path} {A2DP_MIN_VOLUME}",
    ...
    if res and res.returncode == 0 and vol2:
        print(
            "     Set to %d/127 (the volume is controlled by the local mixer from now on)"
            % vol2
        )
        return "volume=%d/127 softvol=%s" % (vol2, _soft_volume_text(soft))
    print(
        "     The setting did not take effect (the volume is still unreadable); not changing SoftVolume again, to avoid interrupting the stream that is playing"
    )
    return note
...
def play_notify_tone(kind, device=None):
    ...
    try:
        ...
        if not dev:
            print(
                "  [提示音] %s：还没有可用的播放设备，跳过（可在电脑上直接看日志）"
                % label
            )
            return False
        ...
        if res is None or res.returncode != 0:
            err = (
                "超时"
                if res is None
                else " ".join(((res.stdout or "") + (res.stderr or "")).split())
            )
            print(
                "  [提示音] %s：在 %s 上播放失败（%s）——常见原因：该声卡正被"
...
def write_test_tone_wav(
    path, seconds=2.0, freq=1000.0, rate=44100, channels=2, side=None
):
    """生成 1kHz 测试音 (WAV)。"""
    with wave.open(path, "wb") as w:
...
def headphone_test():
    ...
    print(
        "  Tip: this self-test needs exclusive access to the sound card. If the bt-mic service is running and a device is connected, first run"
    )
    print(
        "        sudo systemctl stop bt-mic and then run this command (systemctl start when done) -"
    )
    print(
        "        otherwise the service's capture/playback streams and the tests here will fight each other over the sound card, and the results will be wrong"
    )
    ...
    print(
        "  If the headphones are already plugged into the other jack (HAT 3.5mm <-> Raspberry Pi onboard 3.5mm),"
    )
    print(
        "  then the currently selected %s is not the card the headphones are on." % dev
    )
    try:
        ans = (
            input(
                "  Play a test tone on each sound card to find out which one the headphones are plugged into? (y/N): "
            )
            .strip()
            .lower()
        )
        ...
        ans = "n"
        print(
            "  (non-interactive environment: skipping the per-card listening test; you can run sudo python3 %s --find-output separately)"
            % os.path.basename(__file__)
        )
    if ans.startswith("y"):
        find_output_device()
    ...
    if mic_device:
        print(
            f"  -> Microphone: {mic_device} ({mic_channels} channels, capture {MIC_CAPTURE_RATE} Hz)"
        )
        same = same_sound_card(dev, mic_device)
        print(
            "  -> 麦克风与耳机口%s"
            % (
                "**在同一张声卡上**（收发共用 I2S 时钟，只能同一个采样率）"
                if same
                else "是两张不同的声卡（互不影响）"
            )
        )
    hat_mode = decide_hat_mode(dev, mic_device, mic_channels) if mic_device else False
    cross_route = bool(mic_device) and same_sound_card(dev, mic_device)
    print(
        "-- Setting the headphone jack mixer based on the above (including cross routing)--"
    )
    apply_playback_mixer(dev, cross_route=cross_route)

    print_status(
        "Step 1: play 1kHz once for left/right/both channels - listen carefully to which side sounds"
    )
    ...
    for side, label, tip in (
        ...
        time.sleep(0.6)
    print(
        "  ★ Remember which ear you hear in each of these three steps (e.g. only the left ear in all three)"
    )

    print_status(
        "Step 2: objective behavior of the headphone jack in each format (no ears needed)"
    )
    print(
        "  Criterion: underrun=0 and elapsed time ≈ playback duration -> the hardware is running at the declared sample rate for that format;"
    )
    print(
        "        elapsed time clearly too long -> the hardware's actual sample rate does not match what we declared (same-card clock conflict)"
    )
    results = []
    for rate, ch in ((44100, 2), (SAMPLE_RATE, 1), (48000, 2)):
        ...
        results.append(r)
        print(
            "  %5d Hz %d ch -> exit code %s, underrun %d times, elapsed %.1fs (expected 2.0s)%s"
            % (
                rate,
                ch,
                r["rc"],
    ...
    if not mic_device:
        ...
        for cap_rate in (SAMPLE_RATE, 44100, 48000):
            ...
            print(
                "  Capture %5d Hz -> %s"
                % (cap_rate, lvl or "cannot open / cannot read")
            )
        print(
            "  Note: only sample rates that capture a normal level (not digital silence) are usable;"
        )
        print(
            "        If 44100 Hz can be captured, we recommend HEADPHONE_CAPTURE_RATE=44100:"
        )
        print(
            "        playback stays 44.1 kHz stereo and the microphone captures at the same rate, so neither direction goes silent"
        )

    print_status(
        "Step 4: record and play at the same time (play audio while the microphone is really running)"
    )
    print(
        "  Each round starts recording first (-d 30 limits the time and makes sure the device is released at the end), then plays 2 seconds of test tone;"
    )
    print(
        "  A 'Device or resource busy' on the recording side means capture cannot open (the device is occupied),"
    )
    print(
        "  more underruns or a longer elapsed time on the playback side means the two directions are fighting over the same clock"
    )
    if not mic_device:
        print("  (no usable microphone found, skipping)")
    else:
            ...
            for rate, ch in ((rate, ch) for rate, ch in ((44100, 2), (SAMPLE_RATE, 1))):
                ...
                for line in cap_out.splitlines():
                    ...
                    if "busy" in low or "error" in low or "overrun" in low:
                        ...
                        break
                print(
                    "  Capture %5d Hz + playback %5d Hz %d ch -> playback underrun %d times, "
                    "elapsed %.1fs%s%s"
                    % (
                        cap_rate,
                        rate,
...
def print_playback_choices(playback_device, mic_device):
    ...
    for card, dev, label in cards:
        ...
        if mic_card is not None and str(card) == mic_card:
            mark += "  [与麦克风同一张声卡]"
        print(
            "     %-13s %-20s %s%s" % (cand, describe_output_card(label), label, mark)
        )
    ...
    if others:
        ...
        print(
            "     echo 'PLAYBACK_DEVICE=%s' | sudo tee -a %s  然后 sudo systemctl "
            "restart bt-mic" % (others[0][0], CONFIG_FILE)
        )
        print(
            "     （%s：%s；逐张放测试音确认用 sudo python3 %s --find-output）"
...
def list_playback_cards(mic_device=None):
    ...
    if not cards:
        print(
            "  aplay -l lists no playback device at all: onboard audio may be disabled"
        )
        print(
            "  To get sound from the onboard 3.5 mm jack, make sure /boot/firmware/config.txt has dtparam=audio=on"
        )
        return []
    print(
        "-- all playback sound cards (the sound must go to whichever card's jack the headphones are plugged into) --"
    )
...
def find_output_device():
    ...
    """
    print_status(
        "Find which sound card the headphones are plugged into (--find-output)"
    )
    print(
        "  Tip: run sudo systemctl stop bt-mic first, otherwise the service holds the sound card and the test tone may be wrong"
    )
    mic_device = None
    ...
    if mic_device:
        print(
            "  Microphone device: %s (cards on the same sound card as it are marked below)"
            % mic_device
        )
    ...
    for idx, (card, dev, label) in enumerate(cards, 1):
        ...
        if not ok:
            ...
            continue
        print(
            "  [%d] Playing 2.5 seconds of 1 kHz through %s (%s) [LISTEN]..."
            % (idx, cand, label)
        )
        # 先把这张卡的音量拉起来/取消静音:混音为 0 或静音时用户会误判"不是这张卡"
        apply_playback_mixer(cand, quiet=True)
        run(
            f"aplay -D '{cand}' /tmp/bt_find_output.wav",
            check=False,
            timeout=25,
            verbose=False,
        )
        try:
            ans = (
                input("      Did you hear it? (y=this is the one / Enter=no): ")
                .strip()
                .lower()
            )
        except EOFError:
            print(
                "      (non-interactive environment, skipping the prompt: note the card that produced sound and specify it with "
                "PLAYBACK_DEVICE=)"
            )
            continue
        if ans.startswith("y"):
    ...
    if chosen is None:
        ...
        print(
            "     * Headphones in the Pi's onboard 3.5 mm jack -> need dtparam=audio=on (config.txt)"
        )
        print(
            "     * Headphones in the HAT's 3.5 mm jack -> that card is seeed/respeaker/voicecard"
        )
        print("     * Volume/mute: in alsamixer -c <card number>, raise PCM and unmute")
        print(
            "     * Full diagnostics: sudo python3 %s --audio-info"
            % os.path.basename(__file__)
        )
        return
    print(
        "  -> Confirmed to use: %s (%s)"
        % (chosen, dict((f"plughw:{c},{d}", l) for c, d, l in cards).get(chosen, ""))
    )
    ok, info = save_config_key("PLAYBACK_DEVICE", chosen)
    if ok:
        print("  -> Wrote config %s: PLAYBACK_DEVICE=%s" % (info, chosen))
        print(
            "     Takes effect: sudo systemctl restart bt-mic (or rerun this program in the foreground)"
        )
        try:
            ...
            print(
                "\n".join(
                    "       " + l
                    for l in Path(info).read_text(encoding="utf-8").splitlines()
                )
            )
        except OSError:
            pass
        ...
        print(
            "  !! Failed to write the config (%s): please add the line by hand" % info
        )
...
def play_test_tone():
    ...
    if same_card_mic:
        print(
            "  -> This card is also the microphone's sound card: HP cross routing is enabled (both outputs produce sound);"
        )
        print(
            "     to check whether the left/right channels and record-while-play work normally, run sudo python3 %s --hp-test"
            % os.path.basename(__file__)
        )
    ...
    try:
        ...
        print(
            f"  !! Failed to generate the test tone ({exc}), please check whether /tmp is writable"
        )
        return
    print(
        f"  -> Playing a 2 second 1 kHz test tone to {dev} (put on / plug in your headphones)..."
    )
    run(f"aplay -D '{dev}' '{path}'", check=False, timeout=20)
    print(
        "  -> Test finished: hearing the tone means the headphone jack and the mixer are working;"
    )
    print(
        "     if silent, make sure the headphones are in the reSpeaker HAT's 3.5 mm jack and pushed in firmly,"
    )
    print(
        "     and try raising the volume (rerun this command after HEADPHONE_VOLUME=100%)"
    )
...
def print_audio_info():
    ...
    print(
        "  (If the version shown here is older than the script on disk, the service is still running the old process:"
    )
    ...
    print(
        "  Class should be 0x240404 (audio device / wearable headset): Windows uses it to decide"
    )
    print(
        "  which category the Raspberry Pi belongs to, and therefore which endpoints to create (microphone / headphone output)"
    )
    ...
    print(
        "  If A2DP registration fails, check: journalctl -u bluealsa -n 50 --no-pager"
    )
    ...
    for cmd in ("aplay -l", "arecord -l"):
        ...
        res = run(cmd, check=False, timeout=8, verbose=False)
        text = (
            ((res.stdout or "") + (res.stderr or "")) if res else "(execution failed)"
        )
        shown = False
        for line in text.splitlines():
            if line.strip():
    ...
    print(
        f"  Playback devices: {len(pb)}(of which reSpeaker/wm8960 sound cards: {len(hat_pb)})"
    )
    print(
        f"  Recording devices: {len(cap)}(of which reSpeaker/wm8960 sound cards: {len(hat_cap)})"
    )
    if not hat_pb and hat_cap:
        print(
            "  !! The reSpeaker sound card appears only in the recording list (no playback device): that card's playback"
        )
        print(
            "     route is not registered by the driver, so the HAT's 3.5 mm jack cannot play audio"
        )
    elif not hat_pb and not hat_cap:
        print(
            "  !! No seeed/respeaker/wm8960 sound card at all: the HAT driver is not loaded (or no HAT is connected),"
        )
        print(
            "     the 3.5 mm headphone jack on the HAT will not produce sound - only the Pi's onboard 3.5 mm jack will"
        )
    ...
    if dev:
        ...
        if mic_dev_hint and same_sound_card(dev, mic_dev_hint):
            print(
                "  -> The headphone jack and microphone are on the same sound card (TX/RX share the I2S clock, so they can only use one"
            )
            print(
                "     sample rate): the service enables same-card compatibility mode, unifying the headphone downlink to %d Hz %d ch"
                % (_HAT_OUT[0], _HAT_OUT[1])
            )
            print(
                "     When both directions use this card, A2DP stereo drops to that sample rate (trading audio quality for coexistence)"
            )
        else:
            print(
                "  -> The headphone jack and microphone are not on the same sound card: the two directions do not affect each other, so no compatibility mode is needed"
            )
    else:
        print(f"  No usable playback device found: {why}")
    ...
    if not devices:
        ...
        if pcm_list:
            for p in pcm_list:
                ...
                print(
                    "      Manual reproduction (same playback method as inside the program, verifies the BlueALSA -> headphone jack segment):"
                )
                print(
                    "        arecord -D '%s' -f %s -r %d -c %d -t raw | "
                ...
                if dev and same_sound_card(dev, mic_dev_hint):
                    ...
                    print(
                        "      ↑ But the microphone and headphone jack are on the same sound card (TX/RX share the I2S clock),"
                    )
                    print(
                        "        so in practice the service reduces the downlink to this last format before sending it to the headphone jack:"
                    )
                    print(
                        "        arecord -D '%s' -f %s -r %d -c %d -t raw "
                ...
                if prof == "a2dp":
                    print(
                        "      A2DP volume model (consult here when the downlink is scaled to ~0):"
                    )
                    print(
                        "        current: SoftVolume=%s, bluealsa startup args%s --a2dp-volume"
                        % (
                            _soft_volume_text(read_bluealsa_soft_volume(path)),
                            " include"
                            if "--a2dp-volume" in BLUEALSA_EXTRA_ARGS
                            else " do not include",
                        )
                    )
                    print(
                        "        choose one of the two proper approaches (if it is currently false and there is no --a2dp-volume,"
                    )
                    print(
                        "        that is an undefined half-state and it may be scaled by AVRCP volume 0):"
                    )
                    print(
                        "        1) Control the volume on this machine (recommended):"
                    )
                    print("           bluealsa-cli soft-volume %s true" % path)
                    print(
                        "           bluealsa-cli volume %s %d" % (path, A2DP_MIN_VOLUME)
                    )
                    print(
                        "        2) Control the volume with the PC slider: add a drop-in for the service"
                    )
                    print("           Environment=BLUEALSA_EXTRA_ARGS=--a2dp-volume")
                    print(
                        "        also confirm on the PC side: (a) the volume slider for this device in the output list is not at the minimum;"
                    )
                    print(
                        "        (b) in Control Panel -> Sound -> 'Communications' tab, change it to 'Do nothing'"
                    )
                    print(
                        "         - the default setting, when it detects communications activity (we are acting as a microphone), turns other"
                    )
                    print(
                        "        sounds down by 80% or even silences them, which shows up as 'there is sound at first, then no sound'"
                    )
        else:
    ...
    print(
        "  playback method: arecord <downlink PCM> | aplay -D <headphone jack> (built into this program)"
    )
    print(
        "  bluealsa-aplay is not used: its options differ wildly between versions, and in testing some versions"
    )
    print(
        "  have no option to specify the output device, so passing -d exits immediately with `invalid option -- 'd'`"
    )
    res = run(
    ...
    running = [
        l.strip() for l in ((res.stdout or "") if res else "").splitlines() if l.strip()
    ]
    if running:
        ...
        print(
            "  no downlink playback process right now - it appears only after the PC connects and this program starts it;"
        )
        print(
            "  if the service is not running (systemctl status bt-mic), the headphones will make no sound"
        )
    if os.path.exists(HEADPHONE_STATUS_FILE):
        print("  status file %s:" % HEADPHONE_STATUS_FILE)
        try:
            for line in (
                Path(HEADPHONE_STATUS_FILE)
    ...
    if not has_a2dp_arg:
        print(
            "  1) bluealsa was started without A2DP Sink args -> an audio output entry can never appear on the PC."
        )
        print(
            "     confirm bluez-alsa has A2DP support (apt install bluez-alsa-utils),"
        )
        print(
            "     then restart the service so this program registers again: systemctl restart bt-mic"
        )
    elif a2dp_sdp is False:
        print(
            "  1) there is no Audio Sink record in this machine's SDP -> A2DP did not register successfully,"
        )
        print(
            "     check journalctl -u bluealsa for the reason (profile name not supported, etc.)"
        )
    else:
        print(
            "  1) this machine's SDP already has an Audio Sink record (the Raspberry Pi side is fine)"
        )
    if a2dp_now:
        print(
            "  2) the PC is using the A2DP music channel (the Bluetooth link itself is fine). When you hear no"
        )
        print("     sound, check these three places in order:")
        print(
            "     a) the [headphone jack] section above: is the playback device it selected the jack you plugged your headphones into?"
        )
        print(
            "        the onboard jack shows bcm2835 Headphones; the HAT jack shows seeed/wm8960."
        )
        print(
            "        choosing the wrong jack (e.g. headphones plugged into the HAT but the sound sent to the onboard jack) means silence."
        )
        print(
            "     b) run sudo python3 %s --play-test: plays a 1kHz test tone directly on this machine."
            % os.path.basename(__file__)
        )
        print(
            "        sound = the headphone jack and mixer are fine, the problem is on the Bluetooth side; no sound = jack or mixer problem"
        )
        print(
            "     c) 'volume=0 / unreadable' for A2DP in the PCM line above: for A2DP BlueALSA turns on soft volume"
        )
        print(
            "        by default, so samples are scaled by that volume; 0 or unreadable means the whole stream is silent"
        )
        print(
            "        (the service automatically raises it to %d while running; the manual reproduction command is listed above)"
            % A2DP_MIN_VOLUME
        )
    else:
        print(
            "  2) the PC has not established an A2DP music channel yet. Windows only creates an audio output endpoint for the device when A2DP (Audio"
        )
        ...
        print(
            "          or in Windows: Settings -> Bluetooth & other devices -> remove the device -> add it again"
        )
        print(
            "       B. Also confirm these three items on the Windows side (each one can make the output endpoint disappear):"
        )
        print(
            "          * Settings -> Bluetooth & other devices -> More Bluetooth options:"
        )
        print(
            "            check 'Allow Bluetooth devices to play audio' (unchecking it disables all Bluetooth audio output)"
        )
        print(
            "          * Control Panel -> Devices and Printers -> right-click this device -> Properties -> 'Services':"
        )
        print("            check 'Audio Sink' and 'Remote Control',")
        print(
            "            uncheck 'Handsfree' then disconnect and reconnect (this step often forces the output entry out)"
        )
        ...
        print(
            "     after re-pairing, 'Sound settings -> Output' will show two entries for this device:"
        )
        print(
            "       'Headphones (Device name Stereo)'       = A2DP, 44.1/48kHz stereo"
        )
        print("       'Headphones (Device name Hands-Free...)'  = HFP, 16kHz mono")
        print(
            "     if you cannot find it, open the legacy panel: Win+R -> mmsys.cpl -> Playback/Recording"
        )
        print(
            "     (right-click a grayed-out or disabled device to 'Enable' it; you can also 'Show Disconnected Devices' first)"
        )
...
def verify_mic(dev, channels=CHANNELS):
    ...
    if result and result.returncode == 0:
        ...
        if level and "level=silence" in level:
            print(
                f"  !! Microphone {dev} can be opened, but what it captured is "
                f"{strip_level_tag(level)}:"
            )
            print(
                "     wrong device selected / input muted / the card's capture route has a problem - check amixer's"
            )
            print(
                "     capture-side switches (such as 'Mic2L/Mic2R', 'PGA') and whether the microphone is connected properly"
            )
        print(
            f"  -> Microphone {dev} is usable ({channels} channels, {rate} Hz"
...
def check_pipeline_startup(procs, logs):
    ...
    for label, p in procs:
        if p.poll() is not None:
            reasons.append(
                f"{label} exited immediately after startup (exit code {p.returncode})"
            )
    if not reasons:
        time.sleep(5)
        for label, p in procs:
            if p.poll() is not None:
                reasons.append(
                    f"{label} exited after startup (exit code {p.returncode})"
                )
    if reasons:
...
def apply_capture_pga(mic_device, gain_spec):
    ...
    print(
        f"  -> analog PGA gain (sound card {card}): {cur if cur else 'unknown'}"
...
def ensure_soft_volume_disabled(pcm_path):
    ...
    if res and "boolean true" in (res.stdout or ""):
        print(
            "  !! SoftVolume=true detected, +VGM volume commands will be ignored, turning it off..."
        )
        run(
        ...
        print(
            "  -> SoftVolume is now off; the Windows input volume will genuinely affect the capture gain"
        )
        ...
        if pcm_path:
            ...
            if m:
                ...
                if last_level is None:
                    print(
                        f"  [volume] current SCO microphone gain: {level}/15 ({pct}%)"
                    )
...
def print_denoise_status():
    ...
    if os.path.exists(DENOISE_STATUS_FILE):
        ...
        print(
            "  Not recorded yet: the bt-mic service is not running, or the noise reduction mode has not been decided yet"
        )
    ...
    if os.path.exists(HEADPHONE_STATUS_FILE):
        ...
        print(
            "  Not recorded yet: the bt-mic service is not running, or the headphone output has not started"
        )
...
def print_df_info():
    ...
    print("\nNotes:")
    print(
        "  - '0.5.x API (init_df/enhance): True' plus torch available and the model downloaded"
    )
    print(
        "    = the script can enable DeepFilterNet (48kHz model, built-in 16k<->48k resampling)"
    )
    print(
        "  - API present but torch missing or the model not downloaded = install/download it:"
    )
    print(
        "    pip install torch; run init_df once with network access, or place the model zip manually"
    )
    print("  - init_df/enhance and init_model/process both False = no usable df")
    print(
        "    package detected (the script falls back to lightweight spectral subtraction and still works)"
    )
    ...
    with _DF_DECIDE_LOCK:
        ...
        if DENOISE_MODE == "df":
            if has_module("df"):
                ...
                print(
                    "  !! DENOISE_MODE=df but DeepFilterNet was not detected, degrading"
                )
        ...
        if DENOISE_MODE == "spec":
            if has_module("numpy"):
                ...
                print(
                    "  !! numpy is missing, lightweight noise reduction is unavailable, falling back to passthrough forwarding"
                )
                _DF_MODE = None
            _write_denoise_status(_DF_MODE)
            return _DF_MODE
        ...
        if has_module("df"):
            ...
            if rtf_map:
                ...
                print(
                    "  !! DeepFilterNet benchmark failed, switching to lightweight spectral subtraction automatically"
                )
        elif has_module("numpy"):
            print(
                "  -> DeepFilterNet not detected, using lightweight spectral subtraction"
            )
        else:
            print(
                "  -> No noise-reduction dependencies detected, using passthrough forwarding (no noise reduction)"
            )
            _DF_MODE = None
            _write_denoise_status(None)
            return _DF_MODE
...
def _degrade_denoise_mode(venv_python):
    ...
    print(
        "  -> Noise reduction mode degraded to: %s"
        % (_DF_MODE or "no noise reduction (passthrough forwarding)")
    )
...
def run_audio_forwarding(bluealsa_pcm, mic_device, mic_channels, device, warmup=None):
    ...
    """
    print_status(
        "Start audio forwarding (keeps running while the Bluetooth connection stays up)"
    )
        ...
        while True:
            ...
            if not healthy:
                ...
                if restart_count > MAX_RESTARTS:
                    print(
                        "  !! Startup failed repeatedly, returning to the waiting state"
                    )
                    terminate_all(procs)
                    return "failed"
                terminate_all(procs)
                print(
                    f"  -> Will retry in 3 seconds (attempt {restart_count}/{MAX_RESTARTS})"
                )
                time.sleep(3)
                continue
            restart_count = 0
            ...
            while True:
                ...
                if lost_event.is_set() or time.time() - last_bt_check >= 5:
                    ...
                    if not is_device_connected(device):
                        print(
                            "  !! Bluetooth disconnected, stopping audio forwarding, returning to the waiting state"
                        )
                        log_bt_disconnect_reason()
                        terminate_all(procs)
                        return "disconnected"
                ...
                if mode == "df" and any(
                    ...
                    print(
                        "  -> DeepFilterNet cannot keep up in real time, switching to lightweight spectral subtraction automatically"
                    )
                    terminate_all(procs)
                ...
                if restart_count > MAX_RESTARTS:
                    print(
                        "  !! Maximum restart count exceeded, returning to the waiting state"
                    )
                    terminate_all(procs)
                    return "failed"
                terminate_all(procs)
...
def install_systemd_unit():
    ...
    if status and status.stdout.strip() == "active":
        print(
            "  -> bt-mic.service installed and (re)started (active); it will run automatically on boot"
        )
        print(
            "     the service was restarted this time, so the new script takes effect immediately"
        )
...
def uninstall_systemd_unit():
    ...
    run("systemctl daemon-reload", check=False, verbose=False)
    print(
        "  -> Uninstalled (the script file itself is kept; you can --install again at any time)"
    )
    ...
    print(
        "  -> Bluetooth disconnected and powered off (pairing is kept; the next time this program runs, Windows can reconnect directly)"
    )
...
def main():
    ...
    print("的声卡；换了耳机插孔没声音时跑 --find-output 重新确认是哪个孔，或直接写")
    print("  PLAYBACK_PREFER=onboard（树莓派板载 3.5mm）/ hat（reSpeaker HAT）)")
    print(
        "三段提示音区分进度：蓝牙已连接 / 降噪准备完成 / 放音准备完成"
        "（NOTIFY_TONES=0 关闭）"
    )
    ...
    if _CONFIG_APPLIED:
        ...
        print(
            "(no config file %s; to change settings just write KEY=VALUE into it, or run --find-output)"
            % CONFIG_FILE
        )
    check_script_updated()
    ...
    try:
        ...
        while True:
            ...
            if mic_result[0]:
                ...
                break
            print(
                "  !! No recording device found, retrying in 5 seconds (the service keeps running and Bluetooth stays discoverable)"
            )
            ensure_discoverable()
            time.sleep(5)
        ...
        if HEADPHONE_ENABLE:
            ...
            if playback_device:
                ...
                print(
                    "     Bluetooth headphone (speaker) output is unavailable; the microphone is not affected"
                )
        else:
            print(
                "  -> HEADPHONE_ENABLE=0: Bluetooth headphone (speaker) output disabled"
            )
        ...
        while True:
            ...
            if get_connected_devices():
                ...
                print_status(
                    f"Session {session}: device is still connected, trying to resume forwarding directly"
                )
            else:
            ...
            try:
                ...
                while is_device_connected(device):
                    ...
                    if pcm:
                        break
                    print(
                        "  -> The connection is still up but there is no SCO PCM yet, retrying later..."
                    )
                    time.sleep(2)
                if not pcm:
                    print(
                        "  -> Bluetooth disconnected while waiting for the PCM, returning to the waiting state"
                    )
                    stop_denoise_warmup(warmup)
                    continue
                ...
                print(
                    "  -> The program keeps running and Bluetooth is discoverable again, waiting for the next connection"
                )
                time.sleep(3)
            finally:
                hp_stop.set()
...
def warn_wifi_coexistence():
    ...
    for iface in ifaces:
        ...
        if freq < 2500:
            print(
                "  !! WiFi %s 工作在 2.4GHz（channel %s / %d MHz）：本机 WiFi 与"
                "蓝牙共用一颗芯片和天线，" % (iface, m.group(1), freq)
            )
            print(
                "     2.4GHz WiFi 忙时（尤其用 SSH 看日志、传文件）会抢射频，表现"
                "为 SCO 反复停顿（日志里"
            )
            print(
                "     aplay underrun / arecord overrun 十几秒）、声音断续，严重时"
                "蓝牙链路被判超时断开。"
            )
            print("     对策：把 WiFi 换到 5GHz，或直接用有线网口（对调试也更快）。")
            return
        print("  -> WiFi %s 在 5GHz（%d MHz）：与蓝牙的射频争用较小" % (iface, freq))
        return
...
def disable_wifi_powersave():
    ...
    if WIFI_POWERSAVE != "off":
        print(
            "  -> WIFI_POWERSAVE=%s: keeping the system WiFi power-save setting"
            % WIFI_POWERSAVE
        )
        return
    if shutil.which("iw") is None:
        print(
            "  -> iw is not installed, skipping the WiFi power-save change (you can apt install iw)"
        )
        return
    res = run("ls /sys/class/net", check=False, timeout=8, verbose=False)
    ifaces = [
    ...
    for iface in ifaces:
        ...
        if r and r.returncode == 0:
            print(
                f"  -> Turned off WiFi power save on {iface} (reduces Bluetooth audio stutter/dropouts)"
            )
        else:
            print(
                f"  !! Failed to turn off WiFi power save on {iface} (this does not affect other features)"
            )
...
def set_performance_governor():
    ...
    if changed:
        print(
            "  -> CPU governor set to performance (keeps real-time noise reduction speed steadier)"
        )
