...
def configure_bluealsa():
    ...
    if BLUEALSA_EXTRA_ARGS:
        ...
        print(
            "  -> 追加 bluealsa 参数（BLUEALSA_EXTRA_ARGS）: %s" % BLUEALSA_EXTRA_ARGS
        )
    a2dp_candidates = bluealsa_a2dp_profile_name(ba_path) if A2DP_SINK_ENABLE else []
    if not A2DP_SINK_ENABLE:
        print(
            "  -> A2DP_SINK_ENABLE=0：只注册 HFP/HSP（电脑只能用 Hands-Free 通路放音）"
        )
    ...
    if a2dp_profile:
        if a2dp_sdp_ok is True:
            print(
                "  -> 已确认本机 SDP 含 Audio Sink 记录（电脑可把树莓派当耳机/扬声器）"
            )
        else:
            ...
            if a2dp_ok is False:
                print(
                    "  !! 本机 SDP 缺 Audio Sink（A2DP）记录，重启 bluealsa 重新注册..."
                )
                run("systemctl restart bluealsa", check=False, verbose=False)
                time.sleep(3)
                a2dp_ok = verify_a2dp_sdp_record()
            if a2dp_ok is True:
                ...
                print(
                    "  !! 仍缺 Audio Sink SDP 记录：Windows 声音设置里不会有立体声输出项，"
                )
                print(
                    "     耳机口只能走 Hands-Free 下行。请查看 journalctl -u bluealsa。"
                )
...
def select_playback_device(mic_device=None):
    ...
    for card, dev, label, same in ordered:
        ...
        if same:
            ...
            print(
                "     只能同一个采样率（麦克风固定 %d Hz，A2DP 是 44.1/48kHz 会互相挤）"
                % SAMPLE_RATE
            )
            print("     对策（二选一，各试一次）：")
            print(
                "       HEADPHONE_CAPTURE_RATE=44100   麦克风按 44.1kHz 采集（放音保住）"
            )
            print("       HEADPHONE_HAT_MODE=1           把下行降到 16kHz 单声道")
            print("     零取舍的办法：把耳机插到另一张声卡（树莓派板载 3.5mm / USB），")
            print(
                "     或跑 sudo python3 %s --find-output 让它带你确认耳机插在哪张卡"
    ...
    print(
        "  aplay -l 里的播放设备: %s"
        % (", ".join(names) if names else "（一个都没有）")
    )
    if not names:
...
def read_playback_mixer(playback_device):
    ...
    if not found:
        ...
        print(
            f"  声卡 {card} 可用的混音控制: {', '.join(names) if names else '（读不到）'}"
        )
...
def decide_hat_mode(playback_device, mic_device, mic_channels=CHANNELS):
    ...
    if not same_sound_card(playback_device, mic_device):
        print(
            "  -> 麦克风（%s）与耳机口（%s）不在同一张声卡：两个方向不共享时钟，"
    ...
    if opt in ("1", "on", "true", "yes"):
        ...
        for rate, ch, label in ((SAMPLE_RATE, 1, "单声道"), (SAMPLE_RATE, 2, "立体声")):
            ...
            if ok:
                _HAT_OUT[:] = [rate, ch]
                print(
                    "  -> 同卡兼容模式（HEADPHONE_HAT_MODE=%s）：耳机下行统一为 "
                    "%d Hz %s" % (opt, rate, label)
                )
                print(
                    "     注意：本卡 %d Hz 放音实测可能无声（打开成功但听不到），"
                    "若没声请改回 auto 并试 HEADPHONE_CAPTURE_RATE=44100" % SAMPLE_RATE
                )
                print("     自检：%s" % "；".join(tested))
                return True
        print(
            "  !! 耳机口不支持 %d Hz 放音，无法统一到采集采样率；自检：%s"
...
def print_a2dp_silent_hint():
    ...
    print(
        "        （A2DP PCM 路径见 sudo python3 %s --audio-info 的输出）"
        % os.path.basename(__file__)
    )
...
def force_local_a2dp_volume(device):
    ...
    if not path:
        return False
    print(
        "  -> 把 A2DP 下行切到本机控音量：soft-volume true + volume %d"
        % A2DP_MIN_VOLUME
    )
    ...
    vol = read_bluealsa_volume(path)
    print(
        "     现在: SoftVolume=%s 音量=%s（若耳机仍无声，看电平表：电平没变化"
        "就说明幅度是电脑侧压掉的）"
...
def headphone_monitor(
    ...
    print(
        "     播放方式: arecord <下行 PCM> | aplay -D %s%s"
        % (
            playback_device,
            "（HAT 兼容模式：下行统一为 %d Hz 单声道，与麦克风同率）" % SAMPLE_RATE
            if hat_mode
            else "",
        )
    )
        ...
        while not stop_event.is_set():
            ...
            if time.time() - last_a2dp_check >= A2DP_CHECK_SECS:
                ...
                if not active and not no_a2dp_hint and _A2DP_REGISTERED[0]:
                    ...
                    print(
                        "     若输出列表里只有麦克风/没有本设备，说明配对时的服务列表"
                    )
                    print(
                        "     是旧的：在 Windows 删除该设备后重新配对，或在树莓派执行"
                    )
                    print(
                        "     sudo python3 %s --forget 让电脑重新配对。"
                        % os.path.basename(__file__)
                    )
                    print(
                        "     排查用: sudo python3 %s --audio-info"
                        % os.path.basename(__file__)
                    )
            ...
            if not procs:
                ...
                if player is None:
                    if not no_pcm_noted:
                        ...
                        print(
                            "     本机选为输出设备，或还没建立 A2DP/Hands-Free 通路）"
                        )
                    restart = 0
                    stop_event.wait(3)
                    continue
...
def headphone_test():
    ...
    try:
        ans = (
            input("  要逐张声卡放测试音、确认耳机插在哪张吗？（y/N）: ").strip().lower()
        )
    except EOFError:
        ans = "n"
        print(
            "  （非交互环境：跳过逐卡试听；可单独跑 sudo python3 %s --find-output）"
            % os.path.basename(__file__)
        )
    if ans.startswith("y"):
        find_output_device()
    ...
    if mic_device:
        print(
            f"  -> 麦克风: {mic_device}（{mic_channels} 通道，采集 {MIC_CAPTURE_RATE} Hz）"
        )
    ...
    for rate, ch in ((44100, 2), (SAMPLE_RATE, 1), (48000, 2)):
        ...
        results.append(r)
        print(
            "  %5d Hz %d ch -> 退出码 %s，underrun %d 次，耗时 %.1fs（期望 2.0s）%s"
            % (
                rate,
                ch,
                r["rc"],
            ...
            for rate, ch in ((rate, ch) for rate, ch in ((44100, 2), (SAMPLE_RATE, 1))):
                ...
                for line in cap_out.splitlines():
                    ...
                    if "busy" in low or "error" in low or "overrun" in low:
                        ...
                        break
                print(
                    "  采集 %5d Hz + 放音 %5d Hz %d ch -> 放音 underrun %d 次，"
                    "耗时 %.1fs%s%s"
                    % (
                        cap_rate,
                        rate,
...
def list_playback_cards(mic_device=None):
    ...
    if not cards:
        ...
        print(
            "  要在板载 3.5mm 出声，确认 /boot/firmware/config.txt 有 dtparam=audio=on"
        )
        return []
...
def find_output_device():
    ...
    if chosen is None:
        ...
        print(
            "     * 耳机插在树莓派板载 3.5mm 孔 → 需要 dtparam=audio=on（config.txt）"
        )
        print("     * 耳机插在 HAT 的 3.5mm 孔 → 那张卡是 seeed/respeaker/voicecard")
        print("     * 音量/静音：alsamixer -c <卡号> 里把 PCM 调大并取消静音")
        print(
            "     * 完整诊断：sudo python3 %s --audio-info" % os.path.basename(__file__)
        )
        return
    print(
        "  -> 确定使用: %s（%s）"
...
def play_test_tone():
    ...
    if same_card_mic:
        ...
        print(
            "     想确认左右声道/边录边放是否正常，跑 sudo python3 %s --hp-test"
            % os.path.basename(__file__)
        )
...
def print_audio_info():
    ...
    if names is None:
        ...
        for n in names:
            print(f"  - {n}")
        print(
            "  -> Handsfree/Headset（当麦克风需要）: %s" % ("有" if hfp_sdp else "缺失")
        )
        print(
    ...
    if not hat_pb and hat_cap:
        ...
        print(
            "  !! 没有任何 seeed/respeaker/wm8960 声卡：HAT 驱动未加载（或没接 HAT），"
        )
    ...
    if dev:
        ...
        if mic_dev_hint and same_sound_card(dev, mic_dev_hint):
            ...
            print(
                "     采样率）：服务会启用同卡兼容模式，把耳机下行统一为 %d Hz %d ch"
                % (_HAT_OUT[0], _HAT_OUT[1])
            )
    ...
    if not devices:
        ...
        if pcm_list:
            for p in pcm_list:
                ...
                print(
                    "      手工复现（与程序内播放方式相同，验证 BlueALSA → 耳机口 这一段）:"
                )
                print(
                ...
                if prof == "a2dp":
                    ...
                    print(
                        "        当前: SoftVolume=%s，bluealsa 启动参数%s --a2dp-volume"
                        % (
                    ...
                    print("           Environment=BLUEALSA_EXTRA_ARGS=--a2dp-volume")
                    print(
                        "        电脑侧还要确认:（a）输出列表里本设备的音量滑块不在最低;"
                    )
                    print(
                        "        （b）控制面板→声音→『通信』选项卡改成『不执行任何操作』"
                    )
                    print(
                        "        ——默认设置会在检测到通信活动(我们在当麦克风)时把其他"
                    )
                    print(
                        "        声音降低 80% 甚至静音，表现就是『一开始有声后面没声』"
                    )
        else:
    ...
    if a2dp_now:
        ...
        print(
            "     b) 跑 sudo python3 %s --play-test：本机直接放 1kHz 测试音。"
            % os.path.basename(__file__)
        )
...
def verify_mic(dev, channels=CHANNELS):
    ...
    if result and result.returncode == 0:
        ...
        print(
            f"  -> 麦克风 {dev} 可用（{channels} 通道，{rate} Hz"
...
def main():
    ...
    if _CONFIG_APPLIED:
        ...
        print(
            "（无配置文件 %s；改配置可直接往里写 KEY=VALUE，或跑 --find-output）"
            % CONFIG_FILE
        )
    check_script_updated()
