...
float anc_process_fb(float err) {
    ...
    for (int i = 0; i < FB_FILTER_LEN; i++) {
        int pos = (err_idx + 1 + i) % FB_FILTER_LEN;
...
int main() {
    ...
    while (keep_running && rec_count < max_samples) {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            play_buf[i*2]   = anti_out;   /* 左声道 -> 扬声器 */
            play_buf[i*2+1] = 0;          /* 右声道静音 */
    ...
    if (f) { write_wav_header(f, sample_rate, rec_count);
             fwrite(err_rec, sizeof(short), rec_count, f); fclose(f); }
    /* 保存反相波录音 */
    f = fopen("anti_fb.wav", "wb");
    if (f) { write_wav_header(f, sample_rate, rec_count);
             fwrite(anti_rec, sizeof(short), rec_count, f); fclose(f); }
